...
\paragraph{目标函数。}
每次换电交付的净电量为 $E_B(1-\rho_r)$，按服务发生时的价格收费。预测期内的换电收入、充电成本、路径调整成本和预约失败成本分别为
\begin{align}
...
\end{align}
尚未进入高速公路的用户以日前告知的路径为比较基准，在途用户则以最近告知的路径中尚未走过的部分为基准。目标函数计入预计的调整成本，实际费用只在通知用户改变路径时计入一次。

将预测期末的状态输入第 4 节训练的价值网络，得到
\begin{equation}
\Phi^{\mathrm{RL}}=V_\theta(s_{\ell+H}).
\label{eq:rl_terminal_value}
\end{equation}
该值估计预测期结束后的运营净回报，已经计入本轮收入和成本的项目不再重复计算。给定权重 $\beta\ge0$，优化目标为
\begin{equation}
\max_{\mathbf y,\mathbf z}\quad
I-C_{\mathrm{ch}}-C_{\mathrm{adj}}-C_{\mathrm{fail}}+\beta\Phi^{\mathrm{RL}},
\label{eq:objective}
\end{equation}
其中，收入、成本和期末状态都由路径选择、电池分配及以下约束确定。

\paragraph{约束条件。}
（1）\textbf{路径与退回电池的电量。}每位用户在网络中选择一条从本轮起点到出口的路径：
\begin{equation}
...
\label{eq:flow}
\end{equation}
对所有 $p\in\mathcal P$、$k\in\mathcal K_\ell^p$、$n\in\mathcal V^{p,k}$ 成立。网络不含回路，因此满足流平衡的一组 $y$ 就确定了用户依次前往的换电站。对于已在站内等待的用户，连接起点与当前站的固定弧取 $y=1$。

记 $s_{0,\ell}^{p,k}$ 为用户在本轮起点的 SOC：尚未进入高速公路时取预计入口值 $\widetilde{\mathrm{soc}}_{A,\ell}^{p,k}$，已经进入后取当前观测值 $\mathrm{soc}_{A,\ell}^{p,k}$。对请求 $r=(p,k,j,i)\in\mathcal C_i^A$，用户带来的电池 SOC 为
\begin{equation}
...
\label{eq:return_soc}
\end{equation}
首段耗电按本轮起点到站点 $i$ 的路程计算，此后每段行程都从满电开始。随机请求的退回 SOC 取预测值，已经在等待的请求取观测值。

（2）\textbf{路径调整。}对所有 $p\in\mathcal P$、$k\in\mathcal K_\ell^p$ 和 $i\in\mathcal S^p$，用 $x_{A,i}^{p,k}$ 表示用户是否在站点 $i$ 换电，用 $d_{p,k}$ 表示路径是否改变：
\begin{equation}
...
\end{equation}
基准 $x_{A,i\mid\ell}^{\mathrm{ref},p,k}$ 记录此前告知用户的换电站：尚未进入高速公路时采用日前路径，在途时采用最近发布路径中尚未走过的部分。若没有指向站点 $i$ 的弧，则 $x_{A,i}^{p,k}=0$；若沿线没有换电站，则 $d_{p,k}=0$。由于车辆只能向下游行驶，站点确定后，前往各站的顺序也随之确定。因此，只要增加或删除一个换电站，就记为一次路径调整。
