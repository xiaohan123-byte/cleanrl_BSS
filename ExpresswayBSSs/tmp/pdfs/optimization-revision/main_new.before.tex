...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lp{11cm}@{}}
    ...
    $\mathcal N_i,\lambda_{i,\nu}$ & 站点 $i$ 的可行服务模式集合与模式选择变量 \\
...
站点 $i$ 的候选请求集合记为 $\mathcal E_i$，在求解前由已知信息枚举，不以路径变量为成员条件，包括三部分：用户候选网络中指向 $i$ 的弧对应的预约换电事件、预测域内预计到达 $i$ 的随机请求，以及上一轮遗留且仍未超时的等待请求；各请求的到站时刻 $t_r^{\mathrm{arr}}$ 与退回 SOC $\rho_r$ 均为已知参数。其中预约类请求构成子集 $\mathcal E_i^A\subseteq\mathcal E_i$，其余为随机请求；由弧生成的预约事件记为 $\varepsilon=(p,k,j,i)$，表示用户 $(p,k)$ 沿入弧 $(j,i)\in\mathcal A^{p,k}$ 到达 $i$ 的候选换电，其退回 SOC 取 $\rho_\varepsilon=\rho_{A,j,i}^{p,k}$（见式 \eqref{eq:return_soc}）。服务请求用通用索引 $r\in\mathcal E_i$（$i\in\mathcal S$）表示。每个请求分别用以下二元变量表示：$s_r$ 表示在预测域内获得服务，$f_r$ 表示超过等待耐心而失败（预约请求）或流失（随机请求），$\omega_r$ 表示到预测期末仍在等待、带入下一轮；请求的开始服务时刻记为 $t_r^{\mathrm{sw}}$；站点 $i$ 在可行服务模式集合 $\mathcal N_i$ 上的选择记为 $\lambda_{i,\nu}\in\{0,1\}$。此外，$d_{p,k}$ 标记用户 $k$ 剩余行程的换电站路径相对其比较基准是否发生变化。各槽位电池的电量记为 $S_{i,b}(t)$。

每个槽位的请求充电功率 $\widehat P_{i,b,q}$ 与终端价值参数由 RL 给出；电池未满时按当前时段的请求功率线性充电，充满即停；换入用户退回电池后立即恢复充电。
...
\textbf{充电成本。}记 $T_{i,b,q}^{\mathrm{ch}}$ 为槽位 $b$ 在时段 $q$ 内的实际充电时长（扣除已经满电、停止充电的时间），站点 $i$ 在时段 $q$ 的电价为 $c^{ch}_{i,q}$，则预测域内的购电成本为
\begin{equation}
C_{\mathrm{ch}}
=
\sum_{i\in\mathcal S}
...
记本轮路径与服务决策对应的预测终端状态为 $s_{\ell+H}$，其包含期末电池 SOC、未完成请求及其到达时间、截止时间和退回 SOC，以及期末时间、电价和后续需求预测。
...
\label{eq:pwl_critic}
\end{equation}
系数 $\theta=(\boldsymbol a,b,\{c_m,\boldsymbol w_m,d_m\}_{m=1}^{M})$ 由第 4 节的 RL 训练得到，在每轮求解中固定为常数。期末之后的服务收入、充电与路径调整费用及预约超时损失由该价值估计，域内已结算项目不重复计入。$\beta\ge0$ 为终端价值权重。
...
（1）\textbf{网络流平衡约束。}每位预约用户的剩余行程须在其候选网络上构成一条从本轮起点到出口的完整路径，即满足流平衡约束：
\begin{equation}
...
\label{eq:flow}
\end{equation}
其中本轮起点 $o_\ell^{p,k}$ 对未到用户为入口 $o^p$，对在途用户为实时虚拟起点 $o_\ell^{\mathrm{cur},p,k}$；$\mathcal V^{p,k}$ 为该用户剩余候选网络的节点集。路径同时决定各次换电退回电池的电量：用户 $k$ 沿入弧 $(j,i)$ 到达站点 $i$ 时，退回 SOC 按“出发电量减去该段耗电”计算：
\begin{equation}
\rho_{A,j,i}^{p,k}
=
\begin{cases}
...
（2）\textbf{路径调整约束。}剩余路径相对比较基准增加或删除任一换电站，即记一次路径调整。定义当前路径的站点访问量
\begin{equation}
x_{A,i}^{p,k}
=
\sum_{\{j\mid(j,i)\in\mathcal A^{p,k}\}}y_{j,i}^{p,k},
\qquad p\in\mathcal P,\ k\in\mathcal K_\ell^p,\ i\in\mathcal S^p,
...
\qquad
r\in\mathcal E_i,\ i\in\mathcal S,
...
\begin{align}
t_r^{\mathrm{sw}}&\ge t_r^{\mathrm{arr}}s_r,
&&r\in\mathcal E_i,\ i\in\mathcal S,\\
t_r^{\mathrm{sw}}&\le t_r^{\mathrm{ddl}}s_r,
&&r\in\mathcal E_i,\ i\in\mathcal S.
\end{align}
\end{subequations}
每个请求在本轮预测域内的结局恰为以下三种之一：在截止时刻前获得服务（$s_r=1$）、等待超过 $\Delta t$ 而失败或流失（$f_r=1$）、到预测期末仍未超时、留在队列中带入下一轮（$\omega_r=1$）。
\begin{equation}
s_r+f_r+\omega_r=a_r,
\qquad
r\in\mathcal E_i,\ i\in\mathcal S,
...
\end{equation}
其中 $a_r$ 为有效服务量，表示该候选请求确实发生。对随机请求和遗留等待请求，$a_r=1$。对预约事件 $\varepsilon=(p,k,j,i)\in\mathcal E_i^A$，首站事件（入弧起点为本轮起点 $o_\ell^{p,k}$）只取决于路径选择：
\begin{equation}
a_\varepsilon=y_{j,i}^{p,k},
\qquad
\varepsilon=(p,k,j,i)\in\mathcal E_i^A,\ j=o_\ell^{p,k},\ i\in\mathcal S;
\label{eq:first_event_activation}
\end{equation}
其余事件还要求用户在上游站 $j$ 成功换电：
\begin{subequations}
\label{eq:reservation_alive_chain}
\begin{align}
a_\varepsilon&\le y_{j,i}^{p,k},
&&\varepsilon=(p,k,j,i)\in\mathcal E_i^A,\ j\ne o_\ell^{p,k},\ i\in\mathcal S,\\
a_\varepsilon&\le
...
\end{align}
\end{subequations}
其中 $\mathcal E^A(p,k,j):=\{\varepsilon'=(p,k,j',j)\in\mathcal E_j^A\}$ 为用户 $(p,k)$ 在上游站 $j$ 的候选事件集合。这组约束的含义是：用户只有在上游站成功换电，下游事件才有效；一旦上游超时，下游事件全部失效，失败只结算一次；无效事件满足 $s_\varepsilon=f_\varepsilon=\omega_\varepsilon=0$。结局判定还受预测域边界限制：等待窗口完全落在域内的请求不能带入下一轮，窗口跨预测期末的请求本轮不能被判定失败或流失，即
\begin{subequations}
\label{eq:outcome_boundary}
\begin{align}
\omega_r&=0,
&&t_r^{\mathrm{ddl}}<t_{\ell+H},\ r\in\mathcal E_i,\ i\in\mathcal S,\\
f_r&=0,
&&t_r^{\mathrm{ddl}}\ge t_{\ell+H},\ r\in\mathcal E_i,\ i\in\mathcal S.
\end{align}
\end{subequations}

（4）\textbf{充电与换电约束。}本轮各槽位的初始电量、正在充电的状态和遗留等待队列均取真实观测。站内连续过程由事件驱动，事件时刻包括请求到站、换电服务、等待截止、电池充满与时段切换。在相邻事件时刻 $t_1<t_2$ 之间（同属于某个时段 $q$），槽位电量线性上升、充满即止：
\begin{equation}
S_{i,b}(t_2)
=
\min\left\{
...
\end{equation}
即电池未满时按当前时段的功率充电，充满即停；槽位 $b$ 在时段 $q$ 内未满电的累计时长即为充电成本中的实际充电时长 $T_{i,b,q}^{\mathrm{ch}}$。功率的可行范围为
\begin{equation}
...
在上述规则下，站内服务过程是确定的：给定站点 $i$ 的候选请求集合 $\mathcal E_i$、各槽位的初始状态与请求功率序列，服务顺序规则（预约优先于随机、类内按原到站时刻升序、不为未到站请求扣留可用电池）与电量轨迹 \eqref{eq:event_charging_transition} 对候选事件有效性的每种组合，唯一决定各请求的结局与开始服务时刻。据此在求解前枚举站点 $i$ 的可行服务模式集合 $\mathcal N_i$：模式 $\nu\in\mathcal N_i$ 记录有效事件子集 $\mathcal E_\nu^{\mathrm{act}}$、服务集 $\mathcal E_\nu^{\mathrm{s}}$、失败或流失集 $\mathcal E_\nu^{\mathrm{f}}$、期末等待集 $\mathcal E_\nu^{\mathrm{w}}$、各请求的开始服务时刻 $t_{r,\nu}^{\mathrm{sw}}$、期末槽位电量 $S_{i,b,\nu}^{-}$ 与分时段充电时长 $T_{i,b,q,\nu}^{\mathrm{ch}}$；只有满足服务窗口 \eqref{eq:service_window}、满电交付和重新充满要求的结局组合才构成可行模式。每站恰好选择一个模式：
\begin{equation}
\sum_{\nu\in\mathcal N_i}\lambda_{i,\nu}=1,
\qquad
\lambda_{i,\nu}\in\{0,1\},
\qquad i\in\mathcal S,
\label{eq:station_pattern_convexity}
\end{equation}
请求的有效性、结局与开始服务时刻均为所选模式的投影：对 $i\in\mathcal S$、$r\in\mathcal E_i$ 及 $\varepsilon\in\mathcal E_i^A$，
\begin{subequations}
\label{eq:pattern_projection}
\begin{align}
a_\varepsilon&=
\sum_{\nu\in\mathcal N_i:\,\varepsilon\in\mathcal E_\nu^{\mathrm{act}}}
\lambda_{i,\nu},
&
s_r&=
\sum_{\nu\in\mathcal N_i:\,r\in\mathcal E_\nu^{\mathrm{s}}}
\lambda_{i,\nu},
&
f_r&=
\sum_{\nu\in\mathcal N_i:\,r\in\mathcal E_\nu^{\mathrm{f}}}
...
t_r^{\mathrm{sw}}&=
\sum_{\nu\in\mathcal N_i}
t_{r,\nu}^{\mathrm{sw}}\lambda_{i,\nu}.
\end{align}
\end{subequations}
终端库存与充电成本中的实际充电时长同样由所选模式给出：
\begin{equation}
S_{i,b}(t_{\ell+H}^{-})
=
\sum_{\nu\in\mathcal N_i}
...
=
\sum_{\nu\in\mathcal N_i}
T_{i,b,q,\nu}^{\mathrm{ch}}\lambda_{i,\nu},
\qquad i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell.
\label{eq:pattern_state_projection}
\end{equation}
模式投影与有效量链 \eqref{eq:reservation_alive_chain} 共同保证跨站履约一致：若上游站所选模式未服务该用户，其下游事件全部无效。

（5）\textbf{终端状态构造约束。}终端状态 $s_{\ell+H}$ 的库存分量取预测期末事件发生前的槽位电量 $S_{i,b}(t_{\ell+H}^{-})$，由所选服务模式的投影 \eqref{eq:pattern_state_projection} 给出；请求分量为期末仍在等待及未来尚未到站的有效请求，保留其到达时间、截止时间、退回 SOC 与路径信息；期末时间、电价和后续需求预测等外生分量在求解前已知。等待请求按 $\omega_r$ 纳入终端状态；预约换电的期末未完成部分界定如下。令 $\overline{\mathcal D}_{\ell}^{A}$ 为由本轮候选弧和遗留预约请求预先枚举的候选换电集合，$w_h^{\mathrm{pend}}\in\{0,1\}$ 表示其中第 $h$ 项到预测期末仍有效且未完成服务。严格地，令 $A_{p,k}^{\mathrm{bd}}\in\{0,1\}$ 表示用户 $(p,k)$ 在预测域内未发生超时：
\begin{subequations}
...
A_{p,k}^{\mathrm{bd}}
&\le 1-f_\varepsilon,
&&p\in\mathcal P,\ k\in\mathcal K_\ell^p,\ \varepsilon\in\mathcal E_{p,k}^{A,\mathrm{in}},
\end{align}
\end{subequations}
其中 $\mathcal E_{p,k}^{A,\mathrm{in}}$ 为该用户在预测期末前可能到达的候选事件集合，空集时固定为 $1$。令 $\delta_h$ 表示该次换电是否被路径选中（普通候选事件取入弧变量 $y$，遗留请求取 $1$），$s_h^{\mathrm{in}}$ 为其域内服务量（期末前不可能服务的固定为 $0$），则
\begin{subequations}
\label{eq:pending_delivery_alive}
\begin{align}
w_h^{\mathrm{pend}}&\le\delta_h,&
w_h^{\mathrm{pend}}&\le A_{p(h),k(h)}^{\mathrm{bd}},
&&h\in\overline{\mathcal D}_{\ell}^{A},\\
...
\end{align}
\end{subequations}
即期末未完成换电是“路径选中、用户未失效、域内尚未服务”三者的交集。已到站的预约等待请求同时出现在等待队列中，其 $w_h^{\mathrm{pend}}$ 与 $\omega_r$ 取值一致，终端状态对其只编码一次；同一用户尚未完成的下游换电保留其顺序与上游履约依赖。

（6）\textbf{终端价值线性化约束。}在一次 MPC 求解中固定价值网络与编码参数，式\eqref{eq:pwl_critic}中的 ReLU 最大值可逐单元线性化。令 $\zeta_m=\boldsymbol w_m^\top s_{\ell+H}+d_m$，以 $u_m$ 表示其 ReLU 输出。给定有效界 $L_m\le\zeta_m\le U_m$，当 $L_m<0<U_m$ 时，引入 $\delta_m^V\in\{0,1\}$，有
\begin{subequations}
...
训练完成后，actor 在每个滚动时刻输出首期每个槽位的请求功率，并通过式\eqref{eq:rl_power_rollout}生成供 MPC 使用的预测域请求功率序列；实际仅执行首期请求功率。critic 的分段线性参数在本轮求解中固定，MPC 根据自身的路径与服务决策构造 $s_{\ell+H}$，通过式\eqref{eq:pwl_critic_embedding}联合确定终端价值。由此，RL 层同时提供请求功率轨迹和后续运营价值模型，MPC 则优化剩余路径与连续事件安排，形成闭环协同机制；参考推演仅用于生成功率等求解输入，不预先固定本轮终端价值。
