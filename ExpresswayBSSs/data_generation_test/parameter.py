...
4. ``BusinessParameters``：聚合全部业务参数，并提供派生计算方法：
   ...
   - ``travel_time_hours`` / ``travel_periods``：行驶时间 tau_{p,i} 及
     折算的运行时段数；
   - ``arrival_period``：论文 q_{A,i}^{p,k} = [t_A + tau_{p,i}] 的时段归并；
   - ``soc_bin``：按入口 SOC 查询所属离线分档（对应论文候选网络分档）；
   - ``full_soc_tolerance``：将严格的“未满/满电”边界转化为数值稳定的
     MILP 分支；请求功率距补满功率不超过该 SOC 容差时自动补足到 1。
   - ``reservation_failure_penalty``：每次预约换电未能由站内满电库存履约
     时计入的违约成本，使随机需求预测或功率轨迹不再把模型推入不可行。
5. ``get_default_parameters()``：构造可快速求解的非平凡默认实例。

默认实例（plan.md 第 19-20 行）
------------------------------
- 8 个一小时运行时段（索引 0..7），预测时域 H = 4；
- 3 个换电站（索引 0,1,2），各 5 个槽位，位于 80/180/280 km；
- 电池容量 E_B = 100 kWh，续航 300 km（SOC 消耗 v = 距离/300）；
- 车速 75 km/h，时段长度 Delta = 1 h；因此从入口到最近候选首站
  （80 km）的行驶时间约为 1.067 h，暂时规避“用户在决策时刻 ell 前
  已到达首站”的离散时序问题，长期方案见根目录 note.md；
- 2 个 O-D 对，入口均在 0 km，出口分别在 230 km 和 380 km：
  O-D 0 沿线站点为站 0 (80 km)、站 1 (180 km)；
  O-D 1 沿线站点为站 0 (80 km)、站 1 (180 km)、站 2 (280 km)；
- SOC 分档 [0.30,0.50)、[0.50,0.75)、[0.75,1.00]，出口最低 SOC 0.10；
- 满电判定 SOC 容差 epsilon_soc = 1e-6；已补至 SOC 1 的电池必须开放服务；
- 充电效率 0.95，单槽功率上限 60 kW，站级功率上限 240 kW
  （站级时段能量上限 = 站级功率上限 × Delta）；
- 分时电价 [0.35,0.35,0.65,1.10,1.10,0.65,0.40,0.35]（元/kWh，
  各站相同），换电服务价 1.2 元/kWh（各站各时段相同）；
- 路径调整惩罚 kappa = 50，终端价值权重 beta = 1.0，
  最小换电间距下界 D_min = 100 km，随机种子 42；
- 初始逐槽 SOC（确定性给定，不随机生成）：每站 5 槽
  [1.0, 0.9, 0.6, 0.4, 0.2]，3 站相同；
...
0.9333333333333333
>>> params.travel_periods(0, 0, "exit")    # O-D 0 站0(80km)到出口(230km)所需时段数
1.5
...
class StationParameters:
    ...
    num_stations: int = 3
    station_ids: List[int] = field(default_factory=lambda: [0, 1, 2])
    positions_km: List[float] = field(default_factory=lambda: [80.0, 180.0, 280.0])
    num_slots: int = 5
    # 初始逐槽 SOC：initial_slot_soc[i][b] 为站点 i 槽位 b 的初始 SOC。
    initial_slot_soc: List[List[float]] = field(
        default_factory=lambda: [[1.0, 0.9, 0.6, 0.4, 0.2] for _ in range(3)]
    )
    slot_power_limit_kw: float = 60.0  # 单槽充电功率上限 \bar P_i (kW)
    station_power_limit_kw: float = 240.0  # 站级充电功率上限 (kW)
...
class BusinessParameters:
    ...
    # ---- 时段与预测时域 ----
    num_periods: int = 8  # 运行时段总数（索引 0..num_periods-1）
    delta_hours: float = 1.0  # 时段长度 Delta (h)
    ...
    # 用于把“请求功率严格不足”编码为闭合的 MILP 分支。若功率缺口对应的
    # SOC 不超过该值，则执行层自动补足到 1 并令服务就绪指示 g=1。
    full_soc_tolerance: float = 1e-6  # epsilon_soc
    ...
    def validate(self) -> None:
        ...
        if self.delta_hours <= 0:
            raise ValueError(f"delta_hours 必须为正数，当前为 {self.delta_hours}")
        if self.horizon <= 0:
        ...
        if st.station_power_limit_kw <= 0:
            raise ValueError(
                f"station_power_limit_kw 必须为正数，当前为 "
                f"{st.station_power_limit_kw}"
            )
        if st.station_power_limit_kw < st.slot_power_limit_kw:
            raise ValueError(
                f"station_power_limit_kw({st.station_power_limit_kw}) 不得小于 "
                f"slot_power_limit_kw({st.slot_power_limit_kw})"
            )
        ...
        if not 1e-7 <= self.full_soc_tolerance < 1.0:
            raise ValueError(
                "full_soc_tolerance(epsilon_soc) 必须位于 [1e-7,1)，"
                "以显著大于求解器可行性容差；当前为 "
                f"{self.full_soc_tolerance}"
            )
        p_tol = self.full_power_tolerance_kw()
        if p_tol >= st.slot_power_limit_kw:
            raise ValueError(
                f"满电补足功率容差 {p_tol} kW 必须小于单槽功率上限 "
                f"{st.slot_power_limit_kw} kW"
            )
        if st.num_slots * p_tol >= st.station_power_limit_kw:
            raise ValueError(
                f"全站满电补足功率裕量 {st.num_slots * p_tol} kW 必须小于"
    ...
    def travel_periods(
        ...
        return self.travel_time_hours(od_index, from_node, to_node) / self.delta_hours
    ...
    def arrival_period(
        ...
        tau_periods = self.travel_time_from_entry_hours(od_index, node) / self.delta_hours
        return math.floor(entry_time + tau_periods)

    def period_of(self, time_in_hours: float) -> int:
        """连续时刻（小时）归入时段 [q,q+1)，返回时段索引 q。"""
        return math.floor(time_in_hours / self.delta_hours)
    ...
    def power_needed_to_full_kw(self, soc: float, station_index: int = 0) -> float:
        """将 SOC 为 soc 的电池在一个时段内补满所需功率 E_B(1-S)/(eta*Delta)。
        ...
        return (
            ...
            * (1.0 - soc)
            / (self.station.charging_efficiency * self.delta_hours)
        )

    def full_power_tolerance_kw(self, station_index: int = 0) -> float:
        """返回满电 SOC 容差对应的补足功率 p_tol (kW)。
    ...
    def station_energy_limit_kwh(self) -> float:
        """站级时段充电能量上限 \\bar E_{i,q} = 站级功率上限 × Delta (kWh)。"""
        return self.station.station_power_limit_kw * self.delta_hours
    ...
    def from_dict(cls, data: Dict) -> "BusinessParameters":
        ...
        station = StationParameters(**data["station"])
        od_pairs = [ODPairParameters(**od) for od in data["od_pairs"]]
        solver = SolverParameters(**data["solver"])
        rest = {
            k: v
...
def get_default_parameters() -> BusinessParameters:
    """构造 plan.md 第 19-20 行规定的默认实例并通过 validate 校验。"""
    num_periods = 8
    tou_price = [0.35, 0.35, 0.65, 1.10, 1.10, 0.65, 0.40, 0.35]
    station = StationParameters()  # 3 站 × 5 槽，80/180/280 km
    od_pairs = [
        ODPairParameters(od_id=0, entry_km=0.0, exit_km=230.0, station_indices=[0, 1]),
        ODPairParameters(
            od_id=1, entry_km=0.0, exit_km=380.0, station_indices=[0, 1, 2]
        ),
    ]
    params = BusinessParameters(
        ...
        solver=SolverParameters(threads=1, time_limit_sec=30.0, output_flag=0),
    )
    params.validate()
    return params
...
def _print_summary(params: BusinessParameters) -> None:
    ...
    print("=== 业务参数摘要 ===")
    print(
        f"时段: {params.num_periods} 个 × {params.delta_hours} h，预测时域 H = {params.horizon}"
    )
