...
def _station_power_limit(params: Any, station: int, period: int) -> float:
    """读取逐站逐区间功率上限 (kW)，兼容旧参数对象的标量方法。"""
    value = getattr(params, "station_power_limit_kw", None)
    if callable(value):
        try:
    ...
    if value is not None:
        if isinstance(value, (int, float)):
            return _finite_number(value, "station_power_limit_kw")
        try:
            row = value[station]
            item = row[period]
        except (IndexError, KeyError, TypeError) as exc:
            raise MPCInputError(
                f"station_power_limit_kw 缺少站 {station}、区间 {period}"
            ) from exc
        return _finite_number(item, f"station_power_limit_kw[{station}][{period}]")
    raise MPCInputError("参数缺少 station_power_limit_kw")
    ...
    def _validate_event_window(
        ...
        for i in range(n_sta):
            ...
            for h in range(horizon):
                total_power = sum(requested_power[i][b][h] for b in range(n_slot))
                limit = _station_power_limit(p, i, ell + h)
                if limit < 0.0:
                    raise MPCInputError(f"station_power_limit_kw[{i}][{ell+h}] 不能为负")
                if total_power > limit + 1e-9:
                    raise MPCInputError(
                        f"站 {i} 区间 {ell+h} 请求功率 {total_power} kW 超过上限 {limit} kW"
                    )
    ...
    def _event_engine(self, window_engine: Any, grid: Any) -> Any:
        ...
        try:
            return ContinuousEventEngine(
                ...
                station_power_limit_kw=getattr(p, "station_power_limit_kw", None),
            )
        except TypeError:
