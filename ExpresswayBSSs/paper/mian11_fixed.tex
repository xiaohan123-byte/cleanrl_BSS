...
系统服务预约用户和随机到达用户。预约用户在运营日前提交 O-D 对、预计进入高速公路的时刻及入口 SOC，运营者据此决定是否接受预约，并为已接受用户发布日前基准路径。当用户实际进入高速公路时，系统观测其真实进入时刻和 SOC，并可在其到达首个换电站前调整路径；调整后的路径一经发布，尚未完成的换电事件即成为必须履行的承诺。

随机到达用户直接在换电站提出请求，并在预约需求得到满足后按先到先服务规则使用剩余满电电池。各站在每个时段内先充电、后换电，本时段退回的电池从下一时段开始充电。
...
预约用户的 SOC 以高速入口为参考位置，随机到达用户的 SOC 以换电站为参考位置；二者在模型中均转换为换电事件的到站时段和退回 SOC。\autoref{tab:notation}仅列出本节反复使用的核心符号，一次性辅助量均在首次出现处定义。RL 的状态、动作和算法符号在第 3 节定义。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lp{13cm}@{}}
    ...
    $\mathcal{T}_{\ell}$ & 预测时域内的运行时段集合，$\mathcal{T}_{\ell}:=\{\ell+1,\ldots,\ell+H\}$ \\
    $\mathcal{S}^{p},\mathcal{S}$ & O-D 对 $p$ 沿线的换电站集合，以及系统全部换电站的集合 \\
    $\mathcal{B}_{i},b$ & 站点 $i$ 的电池槽位集合及其索引 \\
    $\mathcal R_{i,q},\widehat{\mathcal R}_{\ell,i,q}$ & 时段 $q$ 实际到达站点 $i$ 的随机用户，以及时刻 $\ell$ 对该时段生成的虚拟随机请求 \\
    $\mathcal{V}^{p},n$ & O-D 对 $p$ 的节点集合及一般节点索引；节点包括入口 $o^p$、出口 $d^p$ 和换电站 $\mathcal S^p$ \\
    $\bar{\mathcal A}^{p,k},\mathcal A_{\ell}^{p,k}$ & 分别依据预约入口 SOC 和本轮有效入口 SOC 得到的个体可行弧集 \\
    \midrule
    ...
    $\widetilde t_{A,\ell}^{p,k},\widetilde{\mathrm{soc}}_{A,\ell}^{p,k}$ & 时刻 $\ell$ 采用的有效入口时刻和 SOC \\
    $q_{A,i,\ell}^{p,k}$ & 预约用户 $k$ 在本轮信息下到达节点 $i$ 的时段 \\
    $t_R^{i,u},\mathrm{soc}_R^{i,u}$ & 随机用户 $u$ 的实际到站时刻和 SOC \\
    $\widehat t_{R,\ell}^{i,\nu},\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu}$ & 虚拟随机请求 $\nu$ 的预测到站时刻和 SOC \\
    ...
    $\widehat P_{i,b,q\mid\ell}$ & RL 为槽位 $b$ 请求的充电功率 \\
    ...
    $y_{i,j}^{p,k}$ & 预约用户 $k$ 是否经过可行弧 $(i,j)\in\mathcal A_{\ell}^{p,k}$ \\
    $\alpha_{i,b,\varepsilon,q}$ & 按规范化顺序连接换电事件与电池槽位的匹配辅助变量 \\
    $d_A^{p,k}$ & 路径差异指示量；在最优解中表示当前路径是否偏离日前基准路径 \\
    $\widehat z_{R,\nu,i,q\mid\ell}$ & 虚拟随机请求 $\nu$ 是否在本轮 MPC 预测轨迹中得到服务 \\
    $\delta_\varepsilon$ & 换电事件 $\varepsilon$ 是否在本轮 MPC 预测轨迹中激活 \\
    $P_{i,b,q\mid\ell}$ & 经满电补足与容量校正后的预测可行充电功率 \\
    $S_{i,b,q\mid\ell}^{\mathrm{pre}},S_{i,b,q\mid\ell}$ & 预测轨迹中槽位 $b$ 换电前和换电后的 SOC \\
    \bottomrule
  \end{tabular}
\end{table}
...
运行时段具有固定长度 $\Delta$，并以一个时段为单位对时间轴编号；连续时刻落在区间 $(q-1,q]$ 时归入时段 $q$。设当前滚动决策时刻为 $\ell$，预测时域包含未来 $H$ 个时段，即 $\mathcal T_\ell=\{\ell+1,\ldots,\ell+H\}$。

如\autoref{fig:rolling_horizon}所示，系统在时刻 $\ell$ 已完成上一时段的物理状态更新，并观测区间 $(\ell-1,\ell]$ 内新进入高速公路的预约用户和实际到达各站的随机用户。预约用户的实际入口时刻和 SOC 分别记为 $t_A^{p,k}$ 和 $\mathrm{soc}_A^{p,k}$；随机用户 $u$ 的实际到站时刻和 SOC 分别记为 $t_R^{i,u}$ 和 $\mathrm{soc}_R^{i,u}$。

对于未来时段，需求预测模块生成逐请求的虚拟随机需求。时刻 $\ell$ 生成的虚拟请求 $\nu$ 包含预测到达站点、到站时刻 $\widehat t_{R,\ell}^{i,\nu}$ 和到站 SOC $\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu}$，仅用于构造 MPC 预测轨迹，并不对应某个未来真实用户。RL 层同时给出预测时域内的逐电池请求功率。

运行过程中不再接受新的当日预约。对尚未进入高速公路的已接受用户，$\bar t_A^{p,k}$ 表示系统当前掌握的最新计划入口时刻，其初值为日前预约时刻。迟到用户须通过预约平台更新预计到达时刻；超过预约窗口且未更新的请求按未到场处理。入口 SOC 在用户实际到达前仍采用日前值 $\overline{\mathrm{soc}}_A^{p,k}$。这一规则保证迟到但仍有效的预约不会因原预约时刻已过而从滚动模型中遗漏。
...
\end{equation}
因此，本轮新进入用户使用实际信息调整路径，未来用户使用当前有效预约信息形成前瞻方案。$\mathcal K_\ell^{\mathrm{fix},p}$ 仅包含更早时段已进入高速公路的用户，与 $\mathcal K_\ell^p$ 不重叠。

实际随机用户按其到站时刻归入 $\mathcal R_{i,q}$，预测请求按其预测到站时刻归入 $\widehat{\mathcal R}_{\ell,i,q}$。二者的 SOC 均位于 $[0,1]$，并直接作为换电时的退回 SOC。实际用户索引 $u$ 与虚拟请求索引 $\nu$ 分属不同的信息对象，无需逐一匹配。

$S_{i,b,\ell}^{\mathrm{obs}}$ 表示时刻 $\ell$ 观测到的物理 SOC，$S_{i,b,q\mid\ell}$ 表示基于时刻 $\ell$ 信息预测的 SOC；二者仅在预测轨迹起点相等。MPC 仅使用 $\widehat{\mathcal R}_{\ell,i,q}$ 描述未来随机需求，并联合优化 $\mathcal K_\ell^p$ 中预约用户的完整换电站序列，以计入这些路径对多站点库存的影响。

求解后，系统只向 $\mathcal K_{\ell}^{\mathrm{arr},p}$ 中的本轮新进入用户发布路径；$\mathcal K_{\ell}^{\mathrm{fut},p}$ 的路径不对用户发布，仅用于前瞻评价。

系统随后执行时段 $\ell+1$ 的充电功率和换电服务。时段结束后，预测的随机请求及其服务结果被实际集合 $\mathcal R_{i,\ell+1}$ 取代，物理库存据此更新，未执行的未来预测也在下一滚动时刻重新生成。已发布但尚未完成的预约路径作为固定履约承诺进入后续模型。这一机制可概括为“滚动时域优化、首阶段执行”。

设 $\tau_{p,i}$ 为 O-D 对 $p$ 从入口到节点 $i$ 的行驶时间。预约用户 $k$ 到达节点 $i$ 的时段 $q_{A,i,\ell}^{p,k}$ 是满足下式的唯一整数：
\begin{equation}
\widetilde t_{A,\ell}^{p,k}+\tau_{p,i}
\in
\left(q_{A,i,\ell}^{p,k}-1,q_{A,i,\ell}^{p,k}\right].
\label{eq:user_station_arrival_period}
\end{equation}
本文假设从入口到任一候选首站的行驶时间不少于一个运行时段，且计算与路径发布延迟可以忽略。因此，本轮新进入用户在收到路径后才会到达首个候选站，即
\begin{equation}
...
预约用户的换电站序列用有向网络中的一条入口--出口路径表示。对 O-D 对 $p$，节点集合为
$\mathcal V^p=\{o^p\}\cup\mathcal S^p\cup\{d^p\}$，并按高速公路下游方向排列。若一条路径为
$o^p\rightarrow i_1\rightarrow\cdots\rightarrow i_r\rightarrow d^p$，则用户依次在 $i_1,\ldots,i_r$ 换电；路径跨过的沿线站点不被访问。参数 $D_{i,j}^p$ 表示节点 $i$ 至下游节点 $j$ 的行驶距离，$v_{i,j}^p>0$ 表示相应行程消耗的归一化 SOC。

入口 SOC 决定车辆能够到达的首站。为避免每轮在线求解都从头构造网络，本文在运营前将 $[0,1]$ 划分为少量有序 SOC 档位，并为每个 O-D 对和档位预生成一个候选网络。档位边界仅用于离线索引，不进入在线优化模型，因此不再为其引入额外符号。

生成某一档位的网络时，先按该档位的 SOC 上界收集所有潜在入口弧：车辆电量足以到达某换电站时加入相应的入口--首站弧；电量足以直达出口并满足最低到达 SOC $\underline s_d^p$ 时，加入入口--出口弧。对任一换电站 $i$，若满电车辆能够直接到达出口并满足最低到达 SOC，则只保留 $(i,d^p)$；否则，保留满电车辆能够到达的下游站间弧。由此得到的网络既是该档位内个体可行网络的超集，又保证车辆在换电后已经能够直达出口时不再安排额外换电。

为减少不必要的短间距换电，本文设置期望换电间距 $D^{\mathrm{pref}}>0$。入口--首站弧或站间弧的距离低于该阈值时，算法按距离从小到大尝试删除；仅当删除后仍保留所需的入口--出口连通性时才执行剪枝。对入口弧的检查覆盖同一 SOC 档位内不同的精确可达情形，不能仅因档位上部车辆可直达出口而删除较低 SOC 车辆所需的首站弧。因此，$D^{\mathrm{pref}}$ 是网络简化偏好而非硬约束，站点至出口的末段行程不受该偏好限制。最终仅保存属于至少一条完整入口--出口路径的弧。

在线调用时，系统先根据用户的入口 SOC 选取相应档位的离线网络，再用该用户的精确 SOC 过滤入口弧。若精确入口 SOC 已足以直达出口并满足 $\underline s_d^p$，则入口只保留直达出口弧；否则删除该直达弧，并仅保留实际可达的入口--首站弧。日前弧集 $\bar{\mathcal A}^{p,k}$ 使用预约 SOC $\overline{\mathrm{soc}}_A^{p,k}$ 过滤，滚动弧集 $\mathcal A_{\ell}^{p,k}$ 使用有效 SOC $\widetilde{\mathrm{soc}}_{A,\ell}^{p,k}$ 过滤；过滤后再次删除不属于任何完整入口--出口路径的弧。

上述处理既保留离线预生成的效率，又不会让档位内 SOC 较低的用户选择实际不可达的首站。本文假设已接受用户的有效 SOC 经过过滤后仍存在至少一条可行路径；极端低电量下的道路救援不属于本文讨论范围。
...
\begin{enumerate}
  \item 车辆从入口出发时的 SOC 为 $\overline{\mathrm{soc}}_A^{p,k}$，每次换电后恢复为 $1$。下一节点只能沿日前可行弧集 $\bar{\mathcal A}^{p,k}$ 选择，因此首段和站间行程均满足续航要求。
  \item 若车辆能够从当前位置直接到达出口，且到达 SOC 不低于 $\underline s_d^p$，则终止选站；否则，仅保留预计到站时有满电电池可用、并且仍存在可行下游延续路径的候选站。
...
给定时刻 $\ell$ 的观测库存、预约用户的有效入口信息、未来随机需求预测和 RL 请求功率，MPC 调整预约路径并预测逐电池 SOC 的演化。未来实际随机到达不是已知量，只在首阶段执行后用于更新真实状态。

本文用 $\mathcal B_i$ 表示换电站 $i$ 的电池充电槽。每个时段按照“先充电、后换电”的顺序运行，本时段退回的电池从下一时段开始充电。

MPC 的核心决策是预约用户路径。为表达路径变化、随机服务和逐电池状态转移，模型同时引入若干辅助变量。
...
将某一预约用户或预测随机请求在站点 $i$、时段 $q$ 发起的一次换电需求称为一个换电事件，并用 $\varepsilon$ 对事件进行编号。一个事件被激活，表示对应需求在本轮 MPC 轨迹中接受换电服务；三类换电事件及其激活规则将在约束 4 中统一定义。对每个站点--时段，事件按预约优先、同类事件分别按固定用户编号和预测到站顺序编号；槽位也按编号升序排列。电池分配变量为
\[
\alpha_{i,b,\varepsilon,q}\in\{0,1\}.
\]
若站点 $i$ 的槽位 $b$ 在时段 $q$ 与换电事件 $\varepsilon$ 匹配，则
$\alpha_{i,b,\varepsilon,q}=1$；否则为 $0$。该变量仅用于追踪换电后各块退回电池的 SOC。约束 5 要求激活事件依次使用编号最小的服务就绪槽位，因而给定事件激活量 $\delta$ 和服务就绪指示量 $g$ 后，$\alpha$ 由规范化规则唯一确定，而不是可被目标函数利用的自由匹配选择。真实执行采用同一规则。

模型还引入以下辅助变量。其中，
$\widehat z_{R,\nu,i,q\mid\ell}$ 表示预测随机请求 $\nu$ 是否在本轮 MPC 轨迹中被服务，其取值由预约履约后的剩余满电供给余量和先到先服务顺序确定；
$d_A^{p,k}$ 表示当前路径是否偏离日前基准路径；
$P_{i,b,q\mid\ell}$ 是对 RL 请求进行满电补足与容量校正后的 MPC 轨迹功率；
$S_{i,b,q\mid\ell}^{\mathrm{pre}}$ 和 $S_{i,b,q\mid\ell}$ 分别表示本轮 MPC 预测轨迹中充电后换电前以及换电后的 SOC；
$g_{i,b,q}$ 和 $F_{i,q}$ 分别表示该预测轨迹中的服务就绪指示量和可用电池数。为避免下标过长，$g$、$F$、$Q$、$\alpha$ 和 $\delta$ 省略条件下标“$\mid\ell$”，但在本节 MPC 约束中均不表示真实执行量。
...
\begin{equation}
\max\quad
I_{\ell}^{\mathrm{MPC}}-C_{\mathrm{ch}}-C_{\mathrm{adj}}
+\beta\Phi_{\ell}^{\mathrm{RL}}.
\label{eq:objective}
\end{equation}
其中 $I_{\ell}^{\mathrm{MPC}}$、$C_{\mathrm{ch}}$、$C_{\mathrm{adj}}$ 和 $\Phi_{\ell}^{\mathrm{RL}}$ 分别表示预测轨迹中的换电服务收益、充电成本、路径调整成本和 RL 终端价值，下面分别给出其定义。

\paragraph{Swap-service revenue.}
令 $\pi_{i,q}^{\mathrm{sw}}\ge0$ 表示站点 $i$ 在时段 $q$ 的单位换电电量服务收费，单位为金额/电量。用户获得满电电池并退回 SOC 为 $\rho$ 的电池时，本次换电对应的服务电量为 $E_B(1-\rho)$。因此，预约用户收益与基于随机需求预测计算的前瞻收益分别为
\begin{equation}
\begin{aligned}
I_{\ell}^{\mathrm{MPC}}&=I_{\ell}^{A}+\widehat I_{\ell}^{R},\\
I_{\ell}^{A}
&=E_B\sum_{q\in\mathcal T_\ell}
\sum_{i\in\mathcal S}\pi_{i,q}^{\mathrm{sw}}
\sum_{\varepsilon\in\mathcal E_{\ell,i,q}^{A,\mathrm{dec}}}
(1-\rho_\varepsilon)\delta_\varepsilon,\\
\widehat I_{\ell}^{R}
&=E_B\sum_{q\in\mathcal T_\ell}
\sum_{i\in\mathcal S}\pi_{i,q}^{\mathrm{sw}}
\sum_{\nu\in\widehat{\mathcal R}_{\ell,i,q}}
(1-\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu})
\widehat z_{R,\nu,i,q\mid\ell}.
\end{aligned}
\label{eq:income}
\end{equation}
其中，$I_{\ell}^{A}$ 计入本轮路径决策产生的预约换电收益；已发布固定预约的收益与本轮决策无关，作为常数从目标函数中省略。$\widehat I_{\ell}^{R}$ 使用时刻 $\ell$ 的预测到站信息，因而是前瞻收益而非已实现收益。事件集合、激活量 $\delta_\varepsilon$ 和退回 SOC $\rho_\varepsilon$ 由约束 4 统一定义。个体可行弧集已规定车辆一旦能够满足出口 SOC 要求便直接驶向出口，因此正的服务收益不会诱导可避免的额外换电。
...
$P_{i,b,q\mid\ell}$ 是本轮 MPC 轨迹中包含必要满电补足的可行充电功率，相应的预测时域购电成本为
\begin{equation}
C_{\mathrm{ch}}
=
...
每轮求解前，系统依据固定承诺、当前有效预约信息、随机需求预测和 actor 的均值动作生成一条与本轮路径变量无关的参考轨迹。固定承诺沿已发布路径执行；对 $\mathcal K_\ell^p$ 中的用户，系统在满足个体可达性和参考库存约束的名义路径组合中，先最小化相对日前基准的路径变更数，再按用户与弧编号作字典序选择。该确定性可行性计算与经济目标及本轮路径变量无关。本文假设已接受预约在允许的到达更新范围内至少存在一组这样的名义可行路径；若外生故障或极端偏差破坏该条件，则进入模型范围之外的应急履约机制。参考轨迹的终端状态记为 $s_{\ell+H\mid\ell}^{\mathrm{ref}}$。在该状态处，critic 对槽位 SOC 的边际价值为
\begin{equation}
\lambda^S_{i,b,\ell}
=
\left.
...
{\partial S_{i,b}}
\right|_{s=s_{\ell+H\mid\ell}^{\mathrm{ref}}},
\quad i\in\mathcal S,\ b\in\mathcal B_i.
\label{eq:marginal_value_soc}
\end{equation}
$\lambda^S_{i,b,\ell}$ 表示参考状态附近单位 SOC 增量带来的长期价值。对 critic 作一阶近似并删除与本轮决策无关的常数后，域内终端库存只需保留线性项 $\lambda^S_{i,b,\ell}S_{i,b,\ell+H\mid\ell}$。

对于满足 $q_{A,i,\ell}^{p,k}>\ell+H$ 的预约换电，若入弧 $(j,i)$ 被选择，则站点 $i$ 将在预测时域外发出一块满电电池并接收 SOC 为 $\rho_{A,j,i,\ell}^{p,k}$ 的退回电池。该事件不改变域内终端 SOC，因而需要单独估值。为使反事实在参考终端库存暂时没有满电电池时仍有定义，在站点 $i$ 选择一个规范化槽位：若存在服务就绪槽位，则取编号最小者；否则取 SOC 最高者，并以最小编号打破并列。记 $s_{\ell+H\mid\ell}^{\mathrm{ref},(i,x)}$ 为仅将该槽位 SOC 置为 $x$、其余状态保持不变的参考终端状态，并定义
\begin{equation}
\Delta V_{i,\ell}^{\mathrm{out}}(\rho)
=
 V_\theta\!\left(s_{\ell+H\mid\ell}^{\mathrm{ref},(i,\rho)}\right)
-V_\theta\!\left(s_{\ell+H\mid\ell}^{\mathrm{ref},(i,1)}\right).
\label{eq:outside_swap_value}
\end{equation}
$\Delta V_{i,\ell}^{\mathrm{out}}(\rho)$ 在 MPC 求解前由 critic 计算，并作为路径变量的固定系数。当 $\rho<1$ 且 critic 合理反映库存价值时，该值通常为负，对应未来履约的库存机会成本。上述成对反事实比较的是同一规范化槽位从满电变为退回 SOC 的价值变化，不要求参考轨迹事先在该站保留满电电池；当所选槽位原本已满电时，$s_{\ell+H\mid\ell}^{\mathrm{ref},(i,1)}$ 即与原参考状态一致。
...
\lambda^S_{i,b,\ell}S_{i,b,\ell+H\mid\ell}\\
&+\sum_{p\in\mathcal P}
\sum_{k\in\mathcal K_\ell^p}
\sum_{i\in\mathcal S^p}
\sum_{\substack{j:(j,i)\in\mathcal A_{\ell}^{p,k}\\
q_{A,i,\ell}^{p,k}>\ell+H}}
\Delta V_{i,\ell}^{\mathrm{out}}\!\left(\rho_{A,j,i,\ell}^{p,k}\right)
y_{j,i}^{p,k}.
\end{aligned}
...
每位预约用户满足以下流平衡约束：
\begin{equation}
\sum_{\{j\mid(n,j)\in\mathcal A_{\ell}^{p,k}\}}
y_{n,j}^{p,k}
-
\sum_{\{j\mid(j,n)\in\mathcal A_{\ell}^{p,k}\}}
y_{j,n}^{p,k}
=
\begin{cases}
1,&n=o^p,\\
...
&\Longrightarrow
\begin{cases}
P_{i,b,q\mid\ell}=\widehat P_{i,b,q\mid\ell},\\
\widehat P_{i,b,q\mid\ell}
\le \dfrac{E_B(1-S_{i,b,q-1\mid\ell})}{\eta_i\Delta}
-p_i^{\mathrm{tol}},
\end{cases}
...
\begin{cases}
P_{i,b,q\mid\ell}
=\dfrac{E_B(1-S_{i,b,q-1\mid\ell})}{\eta_i\Delta},\\
\widehat P_{i,b,q\mid\ell}
\ge \dfrac{E_B(1-S_{i,b,q-1\mid\ell})}{\eta_i\Delta}
-p_i^{\mathrm{tol}}.
\end{cases}
\label{eq:complete_charging}
\end{align}
\end{subequations}
第一分支严格执行 RL 请求，并使电池与满电至少相差 $\epsilon_{\mathrm{soc}}$；第二分支把电池恰好补至 SOC $1$。补足功率不超过 $p_i^{\mathrm{tol}}$，并完整计入式\eqref{eq:chargingcost}的充电成本。两分支在请求恰好到达补足边界时均可选择，分别表示推迟完成和立即补足；二者对应不同的实际功率与 SOC，不产生能量记账缺口。日前递推和参考 rollout 在该边界固定采用立即补足分支，以保证参考轨迹唯一；在线 MPC 则保留两种物理可行动作，并将首期 $g^*$ 传递给执行层。式\eqref{eq:power_saturation}可直接写为求解器的线性 indicator 约束。只有 $q=\ell+1$ 的最优首项会被实际执行，其余时段仅用于前瞻评价。充电后的预测 SOC 为
\begin{equation}
S_{i,b,q\mid\ell}^{\mathrm{pre}}
=
S_{i,b,q-1\mid\ell}
+\frac{\eta_i\Delta}{E_B}P_{i,b,q\mid\ell},
...
\label{eq:return_soc}
\end{equation}
首个换电站使用用户进入高速公路时的 SOC $\widetilde{\mathrm{soc}}_{A,\ell}^{p,k}$，后续站点使用上一站换得的满电 SOC $1$。固定承诺用户的退回 SOC
$\rho_{A,j,i}^{\mathrm{fix},p,k}$ 根据已观测入口 SOC 和固定路径按同一公式预先计算；其到站时段 $q_{A,i}^{\mathrm{fix},p,k}$ 是包含时刻 $t_A^{p,k}+\tau_{p,i}$ 的运行时段。预测随机请求
$\nu\in\widehat{\mathcal R}_{\ell,i,q}$ 已按预测到站时刻归入时段 $q$，其预测到站 SOC
$\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu}$ 即为本轮预测轨迹中的退回 SOC。
...
(\widehat z_{R,\nu,i,q\mid\ell},\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu}),
&\varepsilon=\nu\in\mathcal E_{\ell,i,q}^{R}.
\end{cases}
...
\begin{align}
g_{i,b,q}
&\le S_{i,b,q\mid\ell}^{\mathrm{pre}}
\le 1-\epsilon_{\mathrm{soc}}(1-g_{i,b,q}),
&&i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell,\label{eq:service_ready_relation}\\
F_{i,q}
...
\end{align}
\end{subequations}
因此，服务就绪电池的实际换出 SOC、服务电量计费和后续车辆续航均以 $1$ 计算，且该满电量已通过 $P_{i,b,q\mid\ell}$ 计入能量与成本。$g_{i,b,q}$ 是模型内生辅助变量，而非求解器外预先计算的参数。
...
\end{equation}
该约束要求满电电池数覆盖全部预约换电。预约履约后的剩余容量为 $F_{i,q}-Q_{i,q}^{A}$。将 $m=|\widehat{\mathcal R}_{\ell,i,q}|$ 个预测请求按到站时刻排列为 $\nu_1,\ldots,\nu_m$，同一时刻按固定请求编号排序。对第 $r$ 个请求设置
\begin{equation}
r\widehat z_{R,\nu_r,i,q\mid\ell}
\le F_{i,q}-Q_{i,q}^{A}
\le r-1+|\mathcal B_i|\widehat z_{R,\nu_r,i,q\mid\ell},
\quad r=1,\ldots,m.
...
每个激活事件恰好分配一块电池，每块电池在一个时段最多服务一次且必须达到服务就绪阈值。下式中的 $b,b'$ 均属于 $\mathcal B_i$，$\varepsilon,\varepsilon'$ 均属于 $\mathcal E_{\ell,i,q}$：
\begin{subequations}
...
\sum_{\varepsilon\in\mathcal E_{\ell,i,q}}\alpha_{i,b',\varepsilon,q}
+1-g_{i,b',q},
&&i\in\mathcal S,\ q\in\mathcal T_\ell,\ b'<b,\\
...
\begin{align}
\sum_{\varepsilon\in\mathcal E_{\ell,i,q}}\alpha_{i,b,\varepsilon,q}=0
&\Longrightarrow
S_{i,b,q\mid\ell}=S_{i,b,q\mid\ell}^{\mathrm{pre}},
&&i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell,\\
\alpha_{i,b,\varepsilon,q}=1
&\Longrightarrow
S_{i,b,q\mid\ell}=\rho_\varepsilon,
&&i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell,
\ \varepsilon\in\mathcal E_{\ell,i,q}.
\end{align}
\end{subequations}
...
本轮观测库存已吸收区间 $(\ell-1,\ell]$ 内全部实际预约与随机服务结果；预测集合 $\widehat{\mathcal R}_{\ell,i,q}$ 不参与初始状态更新：
\begin{equation}
S_{i,b,\ell\mid\ell}
=
S_{i,b,\ell}^{\mathrm{obs}},
\quad i\in\mathcal S,\ b\in\mathcal B_i.
...
&&i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell,\\
&0\le S_{i,b,q\mid\ell}^{\mathrm{pre}}\le1,
&&i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell,\\
&0\le S_{i,b,q\mid\ell}\le1,
...
每轮求解后，系统仅执行时段 $\ell+1$ 的充电功率及规范化换电匹配，并向 $\mathcal K_\ell^{\mathrm{arr},p}$ 中的新进入用户发布路径。时段结束后，系统根据实际换电结果更新 $S_{i,b,\ell+1}^{\mathrm{obs}}$，将已发布且尚未完成的预约事件转入固定承诺集合，并使用更新后的随机需求预测进入下一轮求解。
...
$f_A(\mathcal Z_{A,\ell})$ 将其编码为固定维度特征。再令 $f_B$、$f_R^{\mathrm{act}}$ 和 $f_R^{\mathrm{pred}}$ 分别编码逐电池 SOC、真实随机到达记录和预测随机请求，则状态表示为
\begin{equation}
\begin{aligned}
s_\ell=\Big(&
f_B\!\left(\{S_{i,b,\ell}^{\mathrm{obs}}\}_{i\in\mathcal S,b\in\mathcal B_i}\right),
f_A(\mathcal Z_{A,\ell}),\\
&
f_R^{\mathrm{act}}\!\left(
\{(i,u,t_R^{i,u},\mathrm{soc}_R^{i,u},z_{R,u,i,\ell}^{\mathrm{act}}):
u\in\mathcal R_{i,\ell}\}_{i\in\mathcal S}
\right),\\
&
f_R^{\mathrm{pred}}\!\left(
\{(i,\nu,\widehat t_{R,\ell}^{i,\nu},\widehat{\mathrm{soc}}_{R,\ell}^{i,\nu}):
...
\begin{equation}
0\le\widehat P_{i,b,q\mid\ell}\le\overline P_i-p_i^{\mathrm{tol}},
\quad
\Delta\sum_{b\in\mathcal B_i}\widehat P_{i,b,q\mid\ell}
\le\overline E_{i,q}-|\mathcal B_i|\Delta p_i^{\mathrm{tol}},
\quad i\in\mathcal S,\ b\in\mathcal B_i,\ q\in\mathcal T_\ell.
\label{eq:rl_action_projection}
\end{equation}
其中 $\overline P_i$ 为站点 $i$ 的单电池功率上限，$\overline E_{i,q}$ 为时段 $q$ 的站级充电能量上限，并假设二者大于相应的预留量。由式\eqref{eq:power_saturation}可知，$P_{i,b,q\mid\ell}\le\widehat P_{i,b,q\mid\ell}+p_i^{\mathrm{tol}}$，因此上述预留保证补足后的逐电池功率和站级总能量仍不超过物理上限。
...
$s_{\ell\mid\ell}^{\mathrm{ref}}=s_\ell$ 出发，利用同一 actor 的均值动作和预测状态递推进行策略 rollout：
\begin{equation}
\{\widehat P_{i,b,q\mid\ell}\}_{i,b}
=
\mu_\phi(s_{q-1\mid\ell}^{\mathrm{ref}}),
\quad q\in\mathcal T_\ell,
\label{eq:rl_power_rollout}
\end{equation}
其中 $\mu_\phi$ 表示策略分布的均值。为避免请求序列生成与本轮待求路径之间形成循环依赖，参考状态 $s_{q\mid\ell}^{\mathrm{ref}}$ 使用已发布固定路径，以及 MPC 终端价值构造中所定义、由当前有效信息确定的名义可行路径组合。参考状态同时使用 $\widehat{\mathcal R}_{\ell,i,q}$ 中的预测到达时间和预测 SOC，但不使用尚待 MPC 求解的 $\widehat z_R$。记 $z_{R,\nu,i,q\mid\ell}^{\mathrm{ref}}$ 为依据参考库存、上述预约路径和同一先到先服务规则逐期生成的名义随机服务指示：先由预测请求数与参考剩余满电数的较小值确定名义服务量，再按预测到站时间确定该指示，并采用与式\eqref{eq:battery_event_assignment}相同的事件顺序和最小可用槽号规则。它只依赖先前参考状态、actor 已给出的请求功率及本轮外生预测，因而与本轮路径变量、$\alpha$ 和 $\widehat z_R$ 无关。参考轨迹继而依据式\eqref{eq:power_saturation}--\eqref{eq:continuous_swap_transition}进行逐电池 SOC 名义转移。

构造后续参考状态时，时段 $q$ 的预测请求及 $z_R^{\mathrm{ref}}$ 进入独立的名义历史编码器 $f_R^{\mathrm{nom}}$；该编码器与 $f_R^{\mathrm{act}}$ 输出维度相同，但不共享输入字段或语义，且只用于参考状态，不写回真实状态。$f_R^{\mathrm{pred}}$ 所需的后续预测窗口由同一需求预测器的条件均值补齐，超出当前 $H$ 步输入的部分采用固定终端预测先验。上述补齐量与本轮路径变量无关。真实集合 $\mathcal R_{i,q}$ 和本轮动态路径均不参与该 rollout。整条请求功率序列作为本轮 MPC 参数，只有首项请求 $\widehat P_{i,b,\ell+1\mid\ell}$ 及其对应的轨迹功率会进入真实执行；下一滚动时刻重新生成序列。

\paragraph{Transition.}
给定 $s_\ell$ 和名义充电序列后，系统求解 MPC，得到逐用户二元路径
$\{y_{i,j}^{p,k*}\}$。仅
$k\in\mathcal K_\ell^{\mathrm{arr},p}$ 的完整换电站序列被正式发布并加入固定承诺集合；
$k\in\mathcal K_\ell^{\mathrm{fut},p}$ 的路径只用于当前预测，不对用户发布，并在下一轮用更新信息重新计算。令 $q=\ell+1$，环境仅执行首期可行功率
\begin{equation}
P_{i,b,q}^{\mathrm{act}}=P_{i,b,q\mid\ell}^{*},
\qquad
...
\end{equation}
首期初始状态和执行功率均与 MPC 首期解一致，因此实际服务就绪指示直接采用 $g_{i,b,q}^{*}$，并令 $F_{i,q}^{\mathrm{obs}}=F_{i,q}^{*}$。记 $\mathcal E_{i,q}^{A,\mathrm{act}}$ 为时段 $q$ 已发布且实际到站的预约换电事件记录集合；每个 $\varepsilon\in\mathcal E_{i,q}^{A,\mathrm{act}}$ 携带由实际入口 SOC 与已发布路径确定的真实退回 SOC $\rho_\varepsilon^{\mathrm{act}}$，且 $Q_{i,q}^{A,\mathrm{act}}=|\mathcal E_{i,q}^{A,\mathrm{act}}|$。在本文确定性行驶时间、无设备故障且已发布预约必须履约的假设下，首期 $q=\ell+1$ 有
\begin{equation}
\begin{gathered}
...
\end{equation}
这一定义使用实际随机集合、实际服务指示与实际 SOC 得到 $\mathbf S_{\ell+1}^{\mathrm{obs}}$；$\widehat{\mathcal R}_{\ell,i,\ell+1}$、$\widehat z_{R,\nu,i,\ell+1\mid\ell}$ 和预测 SOC 均不作为真实到达、真实服务或真实退回 SOC 输入执行算子，但预测可通过 MPC 产生的首期功率与已发布路径间接影响执行。状态转移可表示为
\begin{equation}
s_{\ell+1}
\sim
\mathbb P\!\left(
...
$\widehat{\mathbf P}_\ell=\{\widehat P_{i,b,q\mid\ell}:i\in\mathcal S,b\in\mathcal B_i,q\in\mathcal T_\ell\}$；只有式\eqref{eq:actual_first_step_charging}中的首项功率进入真实物理状态。
...
\sum_{u\in\mathcal R_{i,\ell+1}}
\pi_{i,\ell+1}^{\mathrm{sw}}
(1-\mathrm{soc}_R^{i,u})z_{R,u,i,\ell+1}^{\mathrm{act}}.
\label{eq:realized_random_income}
\end{equation}
其余已实现量为
\begin{align}
I_\ell^{A,\mathrm{act}}
&=E_B\sum_{i\in\mathcal S}\pi_{i,\ell+1}^{\mathrm{sw}}
\sum_{\varepsilon\in\mathcal E_{i,\ell+1}^{A,\mathrm{act}}}
(1-\rho_\varepsilon^{\mathrm{act}}),
\label{eq:realized_reservation_income}\\
C_{\mathrm{ch},\ell}
&=\sum_{i\in\mathcal S}\sum_{b\in\mathcal B_i}
e_{i,\ell+1}\Delta P_{i,b,\ell+1}^{\mathrm{act}},
\label{eq:realized_charging_cost}\\
C_{\mathrm{adj},\ell}
&=\kappa\sum_{p\in\mathcal P}
\sum_{k\in\mathcal K_\ell^{\mathrm{arr},p}}d_A^{p,k*}.
\label{eq:realized_adjustment_cost}
\end{align}
因此，MPC 目标中的预测随机收益 $\widehat I_\ell^R$ 和未来时段成本不会被当作真实 reward；$C_{\mathrm{adj},\ell}$ 也只统计本轮正式发布且偏离日前基准路径的用户。未来用户在 MPC 中的调整成本只是前瞻评价，不会在当期 reward 中重复实现。该 reward 使 actor 学习逐电池连续功率对当前成本、路径稳定性和长期电池状态价值的共同影响。
...
训练完成后，actor 在每个滚动时刻输出首期逐电池请求动作，并通过式\eqref{eq:rl_power_rollout}生成供 MPC 使用的请求功率序列；实际仅执行首期请求经满电补足和容量校正后的物理功率。critic 构造参考终端状态 $s_{\ell+H\mid\ell}^{\mathrm{ref}}$，通过自动微分计算 $V_{\theta}(s)$ 关于每个参考终端 SOC 分量的梯度，得到 $\lambda^S_{i,b,\ell}$；同时按式\eqref{eq:outside_swap_value}评价域外事件用 SOC 为 $\rho$ 的退回电池替换一块满电电池的价值变化。二者作为固定参数进入线性终端价值项 $\Phi_{\ell}^{\mathrm{RL}}$，并作用于预测终端 SOC $S_{i,b,\ell+H\mid\ell}$。由此，RL 层同时向 MPC 提供逐电池请求功率轨迹和长期 SOC 价值，MPC 则反馈逐用户路径、预测换电事件--电池槽分配、轨迹可行功率与预测逐电池状态演化结果，形成闭环协同机制。
