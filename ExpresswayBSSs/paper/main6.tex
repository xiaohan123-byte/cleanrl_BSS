...
为区分服务决策和库存状态，本文设决策时段集合为 $\mathcal{T}=\{1,\ldots,H\}$，电池库存状态时点集合为 $\mathcal{T}^{w}=\{0,1,\ldots,H\}$。其中，预约用户服务变量 $y_{A,i,j}^{n,p,t}$ 和随机用户服务变量 $y_R^{n,i,t}$ 定义在决策时段 $t\in\mathcal{T}$ 上；电池库存变量 $w_{n,i,t}$ 定义在状态时点 $t\in\mathcal{T}^{w}$ 上。对于预约用户，$t$ 表示其出发或指派时段，换电服务发生在其到达相应换电站的时段。因此，预约用户的决策索引仍为出发时段 $t$，而收益、电池消耗和状态转移可通过 $t+\tau_{p,i,t}$ 关联到实际到达时段。

在预约用户服务方面，本文不强制要求所有预约需求均被服务，而是通过字典序优化机制保证预约用户相对于随机用户具有服务优先级：首先在不考虑随机用户服务的情况下最大化可服务预约需求量；随后在保持该最大预约服务量不变的前提下，再优化随机用户服务量、充电成本和终端价值。若实际运营中要求所有成功预约用户必须得到服务，则可在预约接受阶段额外设置容量控制机制，并在 MPC 中将预约服务量固定为全部预约需求。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lp{13cm}@{}}
    ...
    $\mathcal{T}$ & Set of decision periods in the predictive horizon, $\mathcal{T}:=\{1,\dots,H\}$, indexed by $t$ or $k$ \\
    ...
    $\tau_{p,i,t}$ & Travel time from the origin to node $i$ for an EV of O-D pair $p$ departing in period $t$ \\
    ...
    $s_{i,t}$ & Unit swapping service fee at station $i$ during period $t$ \\
...
有限预测时域 MPC 若仅优化预测时域内收益，可能在时域末端过度服务当前用户，从而留下对未来运营不利的库存结构。为缓解这一末端效应，本文利用 RL critic 学到的长期价值信息构造线性终端价值项。RL critic 学习状态价值函数
\begin{equation}
V_{\theta}(s_t)\approx
...
\subsubsection{Reservation-priority mechanism}

仅设置随机用户不超过剩余满电电池数量的约束，并不能严格保证预约用户优先。原因在于预约用户服务量 $y_A$ 本身也是决策变量；如果模型为了服务随机用户而主动降低 $y_A$，则“随机用户使用预约用户之后的剩余容量”这一约束并不能阻止该行为。因此，本文采用字典序优化方式表达预约用户优先级。
...
\end{equation}
第一阶段先在不服务随机用户的情况下最大化可服务预约需求量：
\begin{equation}
S_A^{\star}=\max \left\{S_A(y_A): \text{constraints }\eqref{eq:flow}-\eqref{eq:terminal_inventory}\text{ hold with }y_R=0\right\}.
\label{eq:first_stage_priority}
\end{equation}
第二阶段再求解完整 MPC 经济目标，并增加如下约束：
\begin{equation}
S_A(y_A)=S_A^{\star}.
\label{eq:reservation_priority_fix}
\end{equation}
这样可以保证随机用户服务不会以牺牲预约用户可服务量为代价。若希望采用单阶段近似，也可以令未满足预约需求惩罚系数 $\alpha$ 大于随机用户服务可能带来的最大边际收益，但字典序形式对预约优先级的表达更严格。

\subsubsection{Objective function}

在第二阶段中，目标函数为最大化预测时域内的服务收益，减去未满足预约需求惩罚和电站充电成本，并加入 RL critic 提供的线性终端价值：
\begin{equation}
\max \quad I-C_1-C_2+\beta\Phi^{\mathrm{RL}}_{\ell}.
\label{eq:objective}
\end{equation}

服务收益由预约用户和随机用户完成换电所产生的服务费构成。为保持与预约指派决策的时间逻辑一致，预约用户收益项以出发或指派时段 $t$ 为决策索引，换电服务价格则取用户到达站点 $i$ 的时段 $t+\tau_{p,i,t}$：
\begin{equation}
\begin{aligned}
I=&
\sum_{t\in\mathcal{T}}\sum_{p\in\mathcal{P}}\sum_{n\in\mathcal{N}}
...
\sum_{\{j\mid (j,i)\in\mathcal{A}^{n,p}\}}
 y_{A,j,i}^{n,p,t}
\left(N-\rho_{j,i}^{n,p}\right)s_{i,t+\tau_{p,i,t}}
\\
&+
\sum_{t\in\mathcal{T}}\sum_{i\in\mathcal{S}}\sum_{n\in\mathcal{N}}
y_R^{n,i,t}(N-n)s_{i,t}.
\end{aligned}
\label{eq:income}
\end{equation}
其中 $N-\rho_{j,i}^{n,p}$ 和 $N-n$ 分别表示预约用户与随机用户在换电时获得的补能量。若服务费按次收取而非按补能量收取，可将上述补能量系数并入 $s_{i,t}$。若 $t+\tau_{p,i,t}$ 超出当前预测时域但仍在可获得的预测信息窗口内，则该价格使用外部预测值；若只统计当前时域内实现收益，则可将对应项递延至后续滚动时域计入。
...
\paragraph{Swapping capacity and priority constraints.}
...
\end{equation}
在字典序约束\eqref{eq:reservation_priority_fix}已经保证预约服务量最大化的前提下，随机用户只能使用预约用户占用之后的剩余满电电池，即
\begin{equation}
\sum_{n\in\mathcal{N}}y_R^{n,i,t}
\le
\left[F_{i,t}-Q_{i,t}^{A,\mathrm{cur}}-Q_{i,t}^{A,\mathrm{his}}\right]^+,
\quad \forall i\in\mathcal{S},\ t\in\mathcal{T}.
\label{eq:priority_residual}
\end{equation}
在实际求解中，由于式\eqref{eq:full_battery_capacity}和库存非负性已经保证 $F_{i,t}-Q_{i,t}^{A,\mathrm{cur}}-Q_{i,t}^{A,\mathrm{his}}\ge 0$，式\eqref{eq:priority_residual}可等价写成线性形式
\begin{equation}
\sum_{n\in\mathcal{N}}y_R^{n,i,t}
\le
F_{i,t}-Q_{i,t}^{A,\mathrm{cur}}-Q_{i,t}^{A,\mathrm{his}},
\quad \forall i\in\mathcal{S},\ t\in\mathcal{T}.
\label{eq:priority_linear}
\end{equation}
需要强调的是，真正保证预约用户优先的是两部分共同作用：式\eqref{eq:reservation_priority_fix}保证随机用户不能降低系统可服务的预约总量，式\eqref{eq:priority_linear}保证在每个站点和时段内随机用户只能使用预约用户服务之后的剩余满电电池。
...
引入 RL 终端价值后，MPC 不再仅依赖固定终端库存下界来处理末端效应。式\eqref{eq:rl_terminal_value} 通过 $\lambda_{i,n,\ell}$ 区分不同站点、不同 SOC 电池在未来运营中的长期边际价值，从而引导 MPC 在预测时域末端保留更有价值的库存结构。与此同时，为保证运行安全，仍可保留如下终端满电电池库存下界作为可选硬约束：
\begin{equation}
...
训练完成后，actor 用于在每个决策时刻输出充电功率动作；critic 则用于提供 MPC 终端价值信息。具体而言，在每轮 MPC 求解前，系统构造参考终端状态 $\bar{s}_{\ell+H}$，并通过自动微分计算 $V_{\theta}(s)$ 关于库存输入 $w_{n,i}$ 的梯度，得到 $\lambda_{i,n,\ell}$。该系数随后作为固定参数进入 MPC 目标函数中的线性终端价值项 $\Phi_{\ell}^{\mathrm{RL}}$。由此，RL 层不仅提供充电功率控制动作，还将长期价值信息反馈给 MPC，形成闭环的 MPC-RL 协同优化机制。
