...
本文采用滚动时域思想刻画换电站运营中的动态决策过程。设当前滚动决策时刻为 $t$。为避免预约用户出发时段与实际换电时段之间存在旅行时间滞后而造成预测域边界问题，本文区分预约指派决策域和库存预测域。具体而言，令预约指派决策域长度为 $H^A$，库存预测域长度为 $H^w$，并定义
\begin{equation}
...
\end{equation}
其中，$\mathcal{T}^{A}$ 用于预约用户指派决策，$\mathcal{T}$ 用于随机用户服务、充电过程和库存状态转移，$\mathcal{T}^{w}$ 用于刻画库存状态。记
\begin{equation}
