...
\label{eq:flow}
\end{equation}
其中本轮起点 $o_\ell^{p,k}$ 对未到用户为入口 $o^p$，对在途用户为实时虚拟起点 $o_\ell^{\mathrm{cur},p,k}$；$\mathcal V^{p,k}$ 为该用户剩余候选网络的节点集。网络无环，因此流平衡确定一条从起点到出口的路径；已在等待的用户通向当前站点的固定弧取 $y=1$。路径同时决定各次换电退回电池的电量：用户 $k$ 沿入弧 $(j,i)$ 到达站点 $i$ 时，退回 SOC 按“出发电量减去该段耗电”计算：
\begin{equation}
\rho_{A,j,i}^{p,k}
=
\begin{cases}
...
\label{eq:return_soc}
\end{equation}
即首段用用户当前电量，之后每段从上一站换得的满电出发。对于新预约请求 $r=(p,k,j,i)\in\mathcal C_i^A$，取 $\rho_r=\rho_{A,j,i}^{p,k}$；随机请求的退回 SOC 取预测值，已在等待的请求取观测值。
...
\end{equation}
其中 $\bar x_{A,i}^{p,k}$ 由日前发布路径汇总，$x_{A,i\mid\ell}^{\mathrm{pub},p,k}$ 由最近一次实际发布路径的未执行站序汇总。其中 $d_{p,k}\in\{0,1\}$，单位调整成本 $c^{adj}>0$。路径差异标记满足
\begin{subequations}
