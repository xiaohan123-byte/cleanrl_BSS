...
\label{eq:income}
\end{equation}
其中 $N-\rho_{j,i}^{n,p}$ 和 $N-n$ 分别表示预约用户与随机用户在换电时获得的补能量，$s$ 为固定单位服务费。若服务费按次收取而非按补能量收取，可将上述补能量系数并入固定服务费参数中。由于 $s$ 不随站点和时间变化，收益函数中不再需要预测未来服务价格；跨越预测时域边界的已指派换电服务则通过后续滚动时域中的历史承诺变量继续进入库存转移。
...
\end{equation}
其中，$S_A(y_A)$ 和 $D_A$ 分别由式\eqref{eq:served_reservation_total} 和式\eqref{eq:reservation_total_demand} 定义。当 $\eta$ 趋近于 1 时，模型更接近对预约用户的刚性服务保障；当 $\eta$ 较低时，模型具有更大的经济优化自由度。该约束与目标函数中的未满足预约惩罚共同体现预约用户的较高服务优先级。
