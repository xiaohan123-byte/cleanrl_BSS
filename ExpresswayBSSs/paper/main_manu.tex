...
\begin{table}[!ht]
  ...
  \begin{tabular}{@{}lp{11cm}@{}}
    ...
    $\mathcal E_{i,l}^A$ & 在本轮预测期内，预约用户在站点 $i$ 提出的所有可能的换电服务请求集合，由 $r=(p,k,j,i)$ 索引 \\
    $\mathcal E_{i,l}^R$ & 在本轮预测期内，随机用户在站点 $i$ 提出的换电服务请求集合，由 $r=(k,i)$ 索引 \\
    $\mathcal E_{i,l}^{W}$ & 在本轮预测期内，等待中的预约用户和随机用户在站点 $i$ 提出的换电服务请求集合，由 $r$ 索引 \\
    $\mathcal E_{i,l}$ & 在本轮预测期内，所有用户在站点 $i$ 提出的所有可能换电服务请求集合，由 $r=(p,k,j,i)$ 索引 \\
...
\end{equation}
$y_{i,j}^{p,k}=1$ 表示预约用户的换电路径经过弧 $(i,j)$，一组满足
流平衡的 $y$ 变量唯一确定该用户依次访问的换电站序列。$z_{r,b}=1$ 表示换电服务请求 $r$ 使用槽位 $b$ 中的满电电池，并将退回电池放入同一槽位。

充电功率 $\widehat P_{i,b,q}$ 由 RL 策略网络给出，作为 MPC 的已知参数。
辅助变量 $s_r$ 表示换电服务请求是否获得服务，$f_r$ 表示换电服务请求是否超时导致服务失败，$\omega_r$ 表示换电服务请求到预测期末是否仍在等待。服务时刻 $t_r^{\mathrm{sw}}$、电池 SOC $S_{i,b}(t)$、充电时长 $T_{i,b,q}^{\mathrm{ch}}$、路径是否调整的指示变量 $d_{p,k}$，以及预测期末状态 $s_{\ell+H}$，可以通过核心决策变量和外生参数确定。

\paragraph{目标函数。}
目标函数由服务收益、充电成本、路径调整成本、预约超时成本和 RL 终端价值五项组成。

\textbf{换电服务收益。}每完成一次换电，站点按服务时刻所在时段的服务价格 $\pi_{i,q(t_r^{\mathrm{sw}})}^{\mathrm{sw}}$ 对换出电量收费：用户取走满电电池、退回电量为 $\rho_r$ 的电池时，本次服务电量为 $E_B(1-\rho_r)$。预测域内的服务收益为
\begin{equation}
I
...
\label{eq:income}
\end{equation}
其中 $q(t)$ 表示时刻 $t$ 所属的时段。对 $s_r=0$ 的请求，上式中的收益项取 $0$，不使用其服务时刻。
