...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lp{13cm}@{}}
    ...
    $\tau_{p,i,k}$ & Travel time from the origin to node $i$ for an EV of O-D pair $p$ departing in period $k$ \\
    ...
    $v^{p}_{j,i}$ & Energy consumption from node $j$ to node $i$ on the path of O-D pair $p$; the initial-SOC dependence is represented by $\rho_{j,i}^{n,p}$ and the feasible arc set $\mathcal{A}^{n,p}$  \\
...
本节给出 MPC 层的数学规划模型。为避免时间索引混淆，本文采用如下约定：$t\in\mathcal{T}$ 表示预测时域内第 $t$ 个运行时段，对应库存状态由 $t-1$ 转移到 $t$；$k\in\mathcal{T}$ 表示预约用户的出发时段。当一辆 O-D 对为 $p$、出发时段为 $k$ 的预约车辆在时段 $t$ 到达站点 $i$ 时，有 $k+\tau_{p,i,k}=t$。RL 层给定的充电功率参数 $P_{n,i,t}$ 作用于时段 $t$，即将时点 $t-1$ 的库存转移到时点 $t$。

MPC 层的决策变量包括预约用户在 SOC-wise expanded network 上的服务流量 $y_{A,i,j}^{n,p,t}$、随机用户服务量 $y_R^{n,i,t}$ 以及各站点不同 SOC 电池库存 $w_{n,i,t}$。当前滚动决策时刻记为 $\ell$，由 RL critic 提供的终端库存边际价值系数 $\lambda_{i,n,\ell}$ 在每轮 MPC 求解前已经确定，因此在优化模型中作为外部参数处理。
...
\label{eq:return_soc}
\end{equation}
其中 $\rho_{j,i}^{n,p}$ 表示初始 SOC 为 $n$ 的预约用户沿弧 $(j,i)$ 到达站点 $i$ 时退回电池的 SOC，$v_{j,i}^{p}$ 表示 O-D 对 $p$ 的路径上从节点 $j$ 行驶至节点 $i$ 的耗电量。引入 $\rho_{j,i}^{n,p}$ 后，弧上耗电量本身不再需要索引初始 SOC；不同初始 SOC 对可达性的影响由 SOC-wise arc set $\mathcal{A}^{n,p}$ 和式\eqref{eq:return_soc} 中的第一种情况共同刻画。第一种情况对应用户从起点携带原始电池行驶至第一个换电站，第二种情况对应用户在前一换电站换得满电电池后继续行驶。
...
\end{align}
式\eqref{eq:current_reservation_swap} 与式\eqref{eq:historical_reservation_swap} 均采用以站点 $i$ 为弧尾的流量，是因为用户在到达站点 $i$ 完成换电后，将从站点 $i$ 沿下一条弧继续行驶；根据站点流平衡，该数量与到达站点 $i$ 并需要换电的预约用户数量一致。
...
\end{equation}
在 MPC 求解时，$\lambda_{i,n,\ell}$ 是固定参数，故式\eqref{eq:rl_terminal_value} 对库存变量保持线性，不会破坏原优化模型的可求解性。若 critic 的库存输入经过归一化处理，则需将梯度按照原始库存单位进行尺度还原后再作为 $\lambda_{i,n,\ell}$ 使用。
...
服务收益由预约用户和随机用户完成换电所产生的服务费构成。预约用户的指派决策在其出发时段 $t$ 做出，变量 $y_{A,i,j}^{n,p,t}$ 表示出发时段为 $t$ 的预约流在站点 $i$ 完成换电后沿弧 $(i,j)$ 继续行驶的服务量；该换电服务发生在车辆到达站点 $i$ 的时段 $t+\tau_{p,i,t}$。因此，预约用户收益按原出发时段索引决策变量，并在到达换电站的时段计取服务费：
\begin{equation}
\begin{aligned}
I=&
\sum_{t\in\mathcal{T}}
...
\sum_{\{j\mid (i,j)\in\mathcal{A}^{n,p}\}}
 y_{A,i,j}^{n,p,t}
 (N-\rho_{j,i}^{n,p})s_{i,t+\tau_{p,i,t}}
\\
&+
...
\label{eq:income}
\end{equation}
其中，预约用户收益项沿用出发时段 $t$ 对决策变量进行索引，避免将已经出发的用户指派推迟到实际到达时段再决策；$t+\tau_{p,i,t}$ 仅用于表示该服务在站点 $i$ 发生并计费的时间。$v_{i,j}^{p}$ 和 $N-n$ 分别表示预约用户沿弧 $(i,j)$ 继续行驶对应的补能量度量和随机用户换电获得的补能量。若服务费按次收取而非按补能量收取，可将上述补能量系数并入 $s_{i,t}$。
...
对于 SOC 状态 $r<N$ 的电池，时点 $t$、站点 $i$ 的库存由四部分组成：当前预测时域内到达并归还 SOC 为 $r$ 电池的预约用户、预测时域开始前已经出发但在当前时域内到达的历史预约用户、当前随机用户归还的 SOC 为 $r$ 电池，以及上一时点经过充电后转移到 SOC $r$ 的站内电池。因此有
\begin{equation}
\begin{aligned}
w_{r,i,t}
=&
...
\end{equation}
等价地，随机用户只能使用满足预约用户换电需求之后的剩余满电电池：
\begin{equation}
\sum_{n\in\mathcal{N}}y_R^{n,i,t}
\le F_{i,t}-Q_{i,t}^{A,\mathrm{cur}}-Q_{i,t}^{A,\mathrm{his}},
\quad \forall i\in\mathcal{S},\ t\in\mathcal{T}.
\label{eq:priority}
\end{equation}
式\eqref{eq:priority} 是线性的；结合较高的未满足预约需求惩罚 $\alpha$，模型会优先将可用满电电池分配给预约用户，再将剩余库存用于服务随机用户。
...
训练完成后，actor 用于在每个决策时刻输出充电功率动作；critic 则用于提供 MPC 终端价值信息。具体而言，在每轮 MPC 求解前，系统构造参考终端状态 $\bar{s}_{t+H}$，并通过自动微分计算 $V_{\theta}(s)$ 关于库存输入 $w_{n,i}$ 的梯度，得到 $\lambda_{i,n,\ell}$。该系数随后作为固定参数进入 MPC 目标函数中的线性终端价值项 $\Phi_{\ell}^{\mathrm{RL}}$。由此，RL 层不仅提供充电功率控制动作，还将长期价值信息反馈给 MPC，形成闭环的 MPC-RL 协同优化机制。
