...
def build_remaining_network(params, state, user, network):
    ...
    if not user.entered:
        ...
        if waiting is None:
            ...
            # A short leg admitted by the offline network can already be
            # published. Keep it while physically reachable, including update
            # rounds, so discretionary pruning never forces a path change.
            protected_first = (origin, current[0]) if current else None
            for arc in short:
                if arc == protected_first:
                    continue
