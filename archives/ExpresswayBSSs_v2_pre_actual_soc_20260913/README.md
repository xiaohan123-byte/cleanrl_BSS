...
- 日前路径只考虑道路和SOC可达性，先最少换电，再按站点位置从出口方向择优。候选网络按SOC档上限生成原始弧；删去过近换电弧前，必须仍有档下限SOC可达的完整路径。若档下限原本无完整可达路径，则不剪过近弧，保留原始完整路径供实际SOC筛选。可直达出口不自动删除其它路径。在线重新剪枝保留既有计划中仍可达的首段，防止冻结路径被删成不可行。
...
修复本次SOC剪枝规则之前的 COPT 切换验证记录：96 项测试全部通过。使用 `python run_mpc.py --time-limit 2 --output outputs/copt_validation_run.json` 完成六站、144 段、48 段预测域的运行；63 轮证明最优，81 轮使用时限内可行解，最大 COPT 相对 gap 为 19.35%。6 个预约全部完成，实际净收益为 1851.336878，账本、能量、价格、SOC 和站点功率核对通过。该运行用于功能验收，默认每轮时限仍为 30 秒；详细记录见 outputs/verification.json，统计报告见 outputs/copt_validation_run_statistics.md。
SOC剪枝修复验证：104项测试通过，论文已编译到 outputs/paper_pruning_check/main.pdf。默认路网中SOC为0.499、0.500、0.599时均有7条完整路径，0.600和0.700时均有11条；0.700用户仍可只在180 km站换电。两个入口SOC分别为0.5、0.7的用户完成六站144轮专项验证（H=24、无随机需求、每轮限时2秒），144轮均最优、2人均完成、无预约失败；其中一个用户进入前选择的短首段在进入后的冻结轮仍保持可行。详见 outputs/lower_soc_pruning_verification.json，复现可使用其中记录的命令。
