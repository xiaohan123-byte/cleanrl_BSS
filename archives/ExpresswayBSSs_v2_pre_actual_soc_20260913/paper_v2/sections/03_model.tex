...
节点集合为 $\mathcal V^p=\{o^p\}\cup\mathcal S^p\cup\{d^p\}$，所有节点沿下游方向排列。以 $D_{i,j}^p$ 和 $v_{i,j}^p$ 表示节点间距离和行驶 SOC 消耗，以 $\underline s_d^p$ 表示出口 SOC 下限。以 $\underline s_h$ 和 $\overline s_h$ 分别表示 SOC 区间 $h$ 的下限与上限。最小换电间距 $\underline D$ 用于优先排除距离过近的首段和站间换电安排；为保留可达路径，必要时允许保留距离小于 $\underline D$ 的弧，出口末段按续航条件处理。对每个 SOC 区间 $h$，按以下四步生成候选网络。
...
\item \textbf{剪枝过近换电弧。}先在原始完整路径中检查，以该 SOC 区间下限 $\underline s_h$ 为入口 SOC 时，是否存在满足首段续航和出口 SOC 下限的完整可达路径。若不存在，则不执行本步骤的删弧，保留原始完整路径供在线按实际 SOC 筛选。若存在，则找出距离小于 $\underline D$ 的首段弧和站间弧，按距离从小到大考察；只有删除该弧后，当前网络中仍存在至少一条以 $\underline s_h$ 为入口 SOC 的完整可达路径，才删除该弧及所有包含该弧的路径，否则保留。
\item \textbf{形成候选网络。}合并最终保留路径中的弧，得到 SOC 区间对应的弧集 $\mathcal A^{h,p}$。
\end{enumerate}

上述规则使用区间上限生成原始网络，使用区间下限检查删弧后是否仍有完整可达路径。若下限 SOC 原本可达，则每次剪枝均保留至少一条下限 SOC 可达的完整路径；同档用户的实际 SOC 不低于下限，也能够沿该路径行驶。若下限 SOC 原本不可达，则不剪枝，实际 SOC 对应的原始可达路径得以保留。因此，只要某用户在原始网络中具有完整可达路径，上述剪枝就不会删除其全部可达路径。这里的可达性仅指道路与车辆续航条件，能否按时获得满电电池仍由后续优化模型决定。

在线使用时，根据用户实际入口 SOC 进一步检查首段续航。对在途用户，删除已经经过的节点，以实时位置建立虚拟起点，按实时 SOC 检查其到下游站点的首段弧。最小换电间距始终以入口或最近一次实际换电位置为基准。对于仍满足实时续航的既有剩余路径，重新检查换电间距时保留其首段弧，以免候选网络的重新剪枝使保留计划或已发布安排在路径冻结轮不可行；是否调整该路径仍由后续路径更新约束决定。对已经在站等待的用户，将当前站作为后续路径起点，后续行程以该次换电完成后的满电状态计算；当前等待请求在第~\ref{sec:model}~节中保留，并与后续服务关联。
