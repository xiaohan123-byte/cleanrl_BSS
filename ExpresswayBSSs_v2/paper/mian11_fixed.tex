...
\begin{figure}[!htbp]
    ...
    \caption{MPC-RL协同优化框架（图中的随机需求均值是逐请求预测集合 $\mathcal R$ 的站点--时段聚合表示；随机实际到达通过状态更新反馈，未在图中单列；站级功率在数学模型中展开为逐电池连续功率向量）}
    \label{fig:MPC-RL}
\end{figure}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lp{13cm}@{}}
    ...
    $\mathcal R_{i,q}$ & 站点 $i$ 时段 $q$ 的随机用户集合：MPC 中为时刻 $\ell$ 生成的预测请求，执行与结算中为实际到达 \\
    ...
    $t_R^{i,\nu},\mathrm{soc}_R^{i,\nu}$ & 随机用户的到站时刻和 SOC（MPC 中取预测值，执行与结算中取实际值） \\
    ...
    $\kappa>0,c^{\mathrm{fail}}>0,\beta\ge0$ & 路径调整惩罚、单次预约换电违约成本和 RL 终端价值权重 \\
    \midrule
    ...
    $a_\varepsilon^A,s_\varepsilon^A,f_\varepsilon^A$ & 预约事件是否仍有效到达、是否由站内满电库存履约，以及是否发生预测违约 \\
    $L_{i,q},\chi_{i,q}^A$ & 预约服务后的满电余量，以及本站本时段是否存在预约缺供 \\
...
如\autoref{fig:rolling_horizon}所示，系统在时刻 $\ell$ 观测区间 $(\ell-1,\ell]$ 内新进入高速公路的预约用户，以及实际到达各站的随机用户及其服务结果，完成上一时段的物理状态更新。预约用户 $k$ 的实际到达入口时刻和 SOC 分别记为 $t_A^{p,k}$ 和 $\mathrm{soc}_A^{p,k}$；随机用户 $\nu$ 的实际到站时刻和 SOC 分别记为 $t_R^{i,\nu}$ 和 $\mathrm{soc}_R^{i,\nu}$。运行过程中不再接受新的当日预约。对于未来时段，对尚未进入高速公路的预约用户，$\bar t_A^{p,k}$ 表示用户的日前预约到达时刻，$\overline{\mathrm{soc}}_A^{p,k}$ 表示用户的日前预约 SOC。对尚未到达的随机用户，二者取预测值，分别表示预测到站时刻和预测到站 SOC。

对 O-D 对 $p$，令
\begin{align}
\mathcal K_{\ell}^{\mathrm{fix},p}
&=\left\{k\in\mathcal K^p:k\text{ 已进入高速公路且已发布路径}\right\},\\
...
\end{align}
其中，$\mathcal K_\ell^{\mathrm{fix},p}$ 表示更早时段已进入高速公路的用户，路径已不再调整；$\mathcal K_{\ell}^{\mathrm{arr},p}$ 表示本轮新进入高速公路、因而已观测到实际信息的预约用户，$\mathcal K_{\ell}^{\mathrm{fut},p}$ 表示预计在预测时域内进入但尚未到达的预约用户。定义本轮需要决策的预约用户的入口信息
\begin{equation}
...
对随机用户，站点 $i$ 在区间 $(\ell-1,\ell]$ 内到站的用户构成时段 $\ell-1$ 的实际到达集合 $\mathcal R_{i,\ell-1}$：他们在到站时段内即完成服务，其到达时刻、SOC 和服务结果在时刻 $\ell$ 均已观测，并已随上一时段的物理状态更新进入观测库存，不再参与本轮决策。对决策时域内的时段 $q\in\mathcal T$，随机需求只能使用预测：$\mathcal R_{i,q}$ 表示时刻 $\ell$ 对时段 $q$ 生成的预测随机请求集合，请求 $\nu$ 的预测到站时刻和 SOC 记为 $t_{R}^{i,\nu}$ 和 $\mathrm{soc}_{R}^{i,\nu}$，并满足 $q=[t_{R}^{i,\nu}]$，与预约用户到站时段的归并方式一致。
...
$a_\varepsilon^A$、$s_\varepsilon^A$、$f_\varepsilon^A$ 和 $L_{i,q}$ 分别表示预约事件仍有效、站内履约、预测违约以及预约服务后的物理满电余量。
...
\label{eq:objective}
\end{equation}
其中 $I^{\mathrm{MPC}}$、$C_{\mathrm{ch}}$、$C_{\mathrm{adj}}$、$C_{\mathrm{fail}}$ 和 $\Phi^{\mathrm{RL}}$ 分别表示换电服务收益、充电成本、路径调整成本、预约换电违约成本和 RL 终端价值，下面分别给出其定义。
...
\label{eq:income}
\end{equation}
其中，$I^{A}$ 只计入由站内物理满电库存实际履约的预约换电；允许违约后，已发布固定预约的收益不再是常数，因此与决策预约一并按 $s_\varepsilon^A$ 计入。$I^{R}$ 使用时刻 $\ell$ 的预测到站信息，因而是前瞻收益而非已实现收益。事件集合、激活量 $\delta_\varepsilon$ 和退回 SOC $\rho_\varepsilon$ 由约束 4 统一定义。个体可行弧集已规定车辆一旦能够满足出口 SOC 要求便直接驶向出口，因此正的服务收益不会诱导可避免的额外换电。
...
\paragraph{Reservation-failure cost.}
当激活预约事件没有获得站内满电电池时，记一次预约换电违约：
\begin{equation}
C_{\mathrm{fail}}
=c^{\mathrm{fail}}
\sum_{q\in\mathcal T}\sum_{i\in\mathcal S}
\sum_{\varepsilon\in
\mathcal E_{i,q}^{A,\mathrm{dec}}\cup
\mathcal E_{i,q}^{A,\mathrm{fix}}}
f_\varepsilon^A.
...
\end{equation}
该项以预约换电事件为结算单位。预测域内的 $f_\varepsilon^A$ 只表示当前信息下的缺供风险并进入本轮 MPC 目标；只有首期实际发生的违约才进入 RL reward 和全天实际汇总，重叠滚动窗口中的预测风险不会重复结算。
...
\end{equation}
$\Delta V_{i}^{\mathrm{out}}(\rho)$ 在 MPC 求解前由 critic 计算，并作为固定系数。当 $\rho<1$ 且 critic 合理反映库存价值时，该值通常为负，对应未来履约的库存机会成本。上述成对反事实比较的是同一规范化槽位从满电变为退回 SOC 的价值变化，不要求参考轨迹事先在该站保留满电电池；当所选槽位原本已满电时，$s_{\ell+H}^{\mathrm{ref},(i,1)}$ 即与原参考状态一致。

允许预约软违约后，入弧被选择并不充分保证用户能存活到预测边界。令 $\mathcal E_u^{A,\mathrm{in}}$ 和 $\mathcal E_u^{A,\mathrm{out}}$ 分别为用户 $u=(p,k)$ 的域内和域外预约事件，并定义边界存活变量 $A_u^{\mathrm{bd}}$。由
\begin{subequations}
\label{eq:boundary_alive}
\begin{align}
A_u^{\mathrm{bd}}
&\ge 1-\sum_{\varepsilon\in\mathcal E_u^{A,\mathrm{in}}}
f_\varepsilon^A,\\
A_u^{\mathrm{bd}}
&\le 1-f_\varepsilon^A,
\qquad \varepsilon\in\mathcal E_u^{A,\mathrm{in}}
\end{align}
\end{subequations}
可知 $A_u^{\mathrm{bd}}=1$ 当且仅当该用户全部域内激活事件均已履约；若域内事件集为空，则固定 $A_u^{\mathrm{bd}}=1$。对决策预约的每个域外事件 $\varepsilon$，以 $y_\varepsilon$ 简记其入弧变量，并定义
\begin{subequations}
\label{eq:outside_event_alive}
\begin{align}
w_\varepsilon^{\mathrm{out}}&\le y_\varepsilon,&
w_\varepsilon^{\mathrm{out}}&\le A_{u(\varepsilon)}^{\mathrm{bd}},\\
w_\varepsilon^{\mathrm{out}}
&\ge y_\varepsilon+A_{u(\varepsilon)}^{\mathrm{bd}}-1.
\end{align}
\end{subequations}
因此 $w_\varepsilon^{\mathrm{out}}=1$ 当且仅当域外入弧被选择且用户未在域内失败。固定承诺的域外事件已包含在参考轨迹中；若其用户在域内失败，则用 $A_u^{\mathrm{bd}}-1$ 撤销该参考价值。
...
&+\sum_{\varepsilon\in\mathcal E^{A,\mathrm{out},\mathrm{dec}}}
\Delta V_{i(\varepsilon)}^{\mathrm{out}}(\rho_\varepsilon)
w_\varepsilon^{\mathrm{out}}\\
&+\sum_{\varepsilon\in\mathcal E^{A,\mathrm{out},\mathrm{fix}}}
\Delta V_{i(\varepsilon)}^{\mathrm{out}}(\rho_\varepsilon)
\left(A_{u(\varepsilon)}^{\mathrm{bd}}-1\right).
\end{aligned}
\label{eq:rl_terminal_value}
\end{equation}
第一项评价预测时域结束时的逐电池库存；第二项只对仍存活的决策预约计入尚未反映在终端 SOC 中的域外换电价值；第三项在固定承诺域内失败时撤销参考轨迹原先包含的域外影响，正常履约时为零。域内换电已通过式\eqref{eq:continuous_swap_transition}进入第一项。多个域外事件的价值采用可加近似，忽略其高阶交互；$\beta$ 用于校准 critic 价值与域内运营金额的尺度。
...
$\nu\in\mathcal R_{i,q}$ 已按预测到站时刻归入时段 $q$，其预测到站 SOC
$\mathrm{soc}_{R}^{i,\nu}$ 即为本轮预测轨迹中的退回 SOC。
...
\end{align}
三类事件分别对应本轮可调整路径产生的预约换电、已经发布的固定预约换电，以及预测随机请求。对每个事件
...
(z_{\nu,i,q},\mathrm{soc}_{R}^{i,\nu}),
&\varepsilon=\nu\in\mathcal E_{i,q}^{R}.
\end{cases}
\label{eq:event_activation_and_soc}
\end{equation}
式\eqref{eq:event_activation_and_soc}中的 $\delta_\varepsilon$ 表示路径层面的候选事件激活量。为避免用户在上游违约后仍在下游重复服务或重复计罚，另定义预约事件的有效到达量 $a_\varepsilon^A$。对决策预约的首站事件，有 $a_\varepsilon^A=y_{o^p,i}^{p,k}$；若事件沿弧 $(j,i)$ 从上游站 $j$ 到达，则令
\begin{subequations}
\label{eq:reservation_alive_chain_dec}
\begin{align}
...
\end{align}
\end{subequations}
其中 $\mathcal E^A(p,k,j)$ 为同一用户到达上游站 $j$ 的候选预约事件集合，所选路径上至多有一个对应服务量为 1。对固定承诺用户，将剩余事件按路径先后记为 $\varepsilon_1,\ldots,\varepsilon_J$，并设置
\begin{equation}
a_{\varepsilon_1}^A=1,
\qquad
a_{\varepsilon_r}^A=s_{\varepsilon_{r-1}}^A,
\quad r=2,\ldots,J.
\label{eq:reservation_alive_chain_fix}
\end{equation}
因此，一次违约会使该用户的下游预约事件失活。预测随机事件的激活量仍由剩余电池供给决定。站点 $i$ 在时段 $q$ 的有效预约换电总量为
\begin{equation}
Q_{i,q}^{A}
=
\sum_{\varepsilon\in
...
\end{align}
\end{subequations}
由于 $0\le S_{i,b,q}^{\mathrm{pre}}\le1$，该双向关系强制 $g=1$ 当且仅当电池达到物理满电状态；模型不能把已满电池主动标为不可用或少报 $F_{i,q}$。因此，服务就绪电池的实际换出 SOC、服务电量计费和后续车辆续航均以 $1$ 计算，且该满电量已通过 $P_{i,b,q}$ 计入能量与成本。$g_{i,b,q}$ 是由物理状态确定的模型内生辅助变量，而非可用于策略性限流的控制量。

当预约需求超过物理满电库存时，直接施加 $Q_{i,q}^{A}\le F_{i,q}$ 会使滚动模型不可行。为显式记录该风险，对每个预约事件定义
\begin{equation}
...
\mathcal E_{i,q}^{A,\mathrm{dec}}\cup
\mathcal E_{i,q}^{A,\mathrm{fix}}.
\label{eq:reservation_service_failure}
\end{equation}
因此，未被所选路径激活或已因上游违约而失活的事件满足 $s_\varepsilon^A=f_\varepsilon^A=0$，不会被误计为违约。令
$Q_{i,q}^{A,\mathrm{serv}}=\sum_\varepsilon s_\varepsilon^A$，并用 $L_{i,q}$ 表示预约服务后的物理满电余量，则预约优先由以下硬约束实现：
\begin{subequations}
\label{eq:reservation_priority}
\begin{align}
Q_{i,q}^{A,\mathrm{serv}}+L_{i,q}
&=F_{i,q},\\
\sum_\varepsilon f_\varepsilon^A
&\le |\mathcal E_{i,q}^{A}|\chi_{i,q}^A,\\
\sum_\varepsilon f_\varepsilon^A
&\ge \chi_{i,q}^A,\\
L_{i,q}
&\le |\mathcal B_i|(1-\chi_{i,q}^A),
\qquad i\in\mathcal S,\ q\in\mathcal T,
\end{align}
\end{subequations}
其中求和均遍历 $\mathcal E_{i,q}^{A}=\mathcal E_{i,q}^{A,\mathrm{dec}}\cup\mathcal E_{i,q}^{A,\mathrm{fix}}$。若存在预约违约，则 $\chi_{i,q}^A=1$ 且 $L_{i,q}=0$，即全部物理满电池必须先分配给预约，随机用户不能越级；若没有违约，则全部激活预约均得到服务，$L_{i,q}=F_{i,q}-Q_{i,q}^{A}$。同站同时段内，预约按“已发布固定承诺、本轮新到、尚未发布未来预约”分级，再按 O-D 和用户编号排序。对任意排在前面的预约 $\varepsilon'$ 和后面的预约 $\varepsilon$，施加
\begin{equation}
s_\varepsilon^A
\le s_{\varepsilon'}^A+1-a_{\varepsilon'}^A,
\qquad \varepsilon'<\varepsilon,
\label{eq:reservation_service_order}
\end{equation}
从而只允许激活预约的服务前缀，未激活候选事件不占顺序位置。

将 $m=|\mathcal R_{i,q}|$ 个预测随机请求按到站时刻排列为 $\nu_1,\ldots,\nu_m$，同一时刻按固定请求编号排序。对第 $r$ 个请求设置
\begin{equation}
rz_{\nu_r,i,q}
\le L_{i,q}
...
\end{equation}
当预约后的物理余量不少于 $r$ 时，右侧不等式强制第 $r$ 个请求得到服务；容量不足时，左侧不等式强制其不被服务。因此，式\eqref{eq:automatic_random_service}实现“有满电池就必须服务”的随机 FCFS 最长前缀，且式\eqref{eq:reservation_priority}保证预约缺供时随机服务量为零。该规则只作用于虚拟预测请求，不把它们视为已经发生的真实到达。

\paragraph{Constraint 5: Matching swap events to service-ready batteries.}
每个实际服务事件恰好分配一块电池，每块电池在一个时段最多服务一次且必须达到服务就绪阈值。下式中的 $b,b'$ 均属于 $\mathcal B_i$，$\varepsilon,\varepsilon'$ 均属于 $\mathcal E_{i,q}$：
\begin{subequations}
...
\begin{cases}
s_\varepsilon^A,
&\varepsilon\in\mathcal E_{i,q}^{A,\mathrm{dec}}\cup
\mathcal E_{i,q}^{A,\mathrm{fix}},\\
z_{\nu,i,q},
&\varepsilon=\nu\in\mathcal E_{i,q}^{R},
\end{cases}
...
\end{align}
\end{subequations}
前两式分别要求每个由站内库存履约的事件恰好获得一块电池，并禁止使用未达到服务就绪阈值的电池。预约违约事件满足 $s_\varepsilon^A=0$，因而不换出站内电池，也不把其退回 SOC 写入库存轨迹。第三式要求只要较小编号的槽位可用，便不能跳过它而使用较大编号的槽位；第四式禁止事件顺序与槽位顺序交叉。因而，实际服务事件依次匹配编号最小的可用槽位，$\alpha$ 仅用于把相应退回 SOC 写入逐电池预测轨迹。第 3 节的真实状态转移采用同一规范化规则。
...
本轮观测库存已吸收区间 $(\ell-1,\ell]$ 内全部实际预约与随机服务结果；预测集合 $\mathcal R_{i,q}$ 不参与初始状态更新：
\begin{equation}
S_{i,b,\ell-1}
=
...
&&i\in\mathcal S,\ q\in\mathcal T,\
\nu\in\mathcal R_{i,q},\\
&a_\varepsilon^A,s_\varepsilon^A,f_\varepsilon^A\in\{0,1\},
&&i\in\mathcal S,\ q\in\mathcal T,\
\varepsilon\in\mathcal E_{i,q}^{A},\\
&A_u^{\mathrm{bd}}\in\{0,1\},
&&u\in\bigcup_{p\in\mathcal P}
\left(\mathcal K_\ell^p\cup\mathcal K_\ell^{\mathrm{fix},p}\right),\\
&w_\varepsilon^{\mathrm{out}}\in\{0,1\},
&&\varepsilon\in\mathcal E^{A,\mathrm{out},\mathrm{dec}},\\
&\chi_{i,q}^A\in\{0,1\},\quad L_{i,q}\in\mathbb Z_+,
&&i\in\mathcal S,\ q\in\mathcal T,\\
&\alpha_{i,b,\varepsilon,q}\in\{0,1\},
&&i\in\mathcal S,\ q\in\mathcal T,\
...
\{(i,\nu,t_{R}^{i,\nu},\mathrm{soc}_{R}^{i,\nu}):
\nu\in\mathcal R_{i,q}\}_{i\in\mathcal S,q\in\mathcal T}
\right),
...
\label{eq:rl_state}
\end{equation}
其中 $\xi_\ell$ 表示时段、节假日或交通高峰等外生特征。$f_R^{\mathrm{act}}$ 使用真实的到达时间、SOC 和服务结果，$f_R^{\mathrm{pred}}$ 仅使用时刻 $\ell$ 可得的预测量，两者不共享数据字段。由于同一站点的库存槽标签没有物理意义，$f_B$ 和 critic 采用可微的置换不变集合编码，actor 的逐槽输出采用相应的置换等变解码；参考 rollout 则按预先固定的对称性消除规则将事件分配给槽位。这样的状态既保留逐电池状态信息，也避免任意槽位编号影响策略和 critic 梯度。
...
具体可用 sigmoid 给出逐电池上界，再对站级总量作可微归一化。策略概率密度及 PPO 比率均针对该可行域参数化后的请求动作 $\widehat P$ 计算；MPC 根据式\eqref{eq:power_saturation}得到包含必要补足的轨迹功率 $P$，其最优首项再按式\eqref{eq:actual_first_step_charging}实际执行。
...
\end{equation}
首期初始状态和执行功率均与 MPC 首期解一致。执行层按物理状态复核
\begin{equation}
\begin{gathered}
g_{i,b,q}^{*}
=\mathbb I\!\left\{
S_{i,b,q}^{\mathrm{pre,obs}}
\ge1-\epsilon_{\mathrm{soc}}/2
\right\},\\
F_{i,q}^{\mathrm{obs}}
=\sum_b g_{i,b,q}^{*}
=F_{i,q}^{*}.
\end{gathered}
\label{eq:first_stage_physical_crosscheck}
\end{equation}
若交叉检查不一致则停止执行而不是少报库存。记 $\mathcal E_{i,q}^{A,\mathrm{act}}$ 为时段 $q$ 已发布且实际到站的预约换电事件记录集合；每个 $\varepsilon\in\mathcal E_{i,q}^{A,\mathrm{act}}$ 携带由实际入口 SOC 与已发布路径确定的真实退回 SOC $\rho_\varepsilon^{\mathrm{act}}$，且 $Q_{i,q}^{A,\mathrm{act}}=|\mathcal E_{i,q}^{A,\mathrm{act}}|$。首期 $q=\ell$ 有
\begin{equation}
\begin{gathered}
...
\qquad
Q_{i,q}^{A,\mathrm{serv,act}}
=\min\{Q_{i,q}^{A,\mathrm{act}},F_{i,q}^{\mathrm{obs}}\},\\
Q_{i,q}^{A,\mathrm{fail,act}}
=Q_{i,q}^{A,\mathrm{act}}-Q_{i,q}^{A,\mathrm{serv,act}},
\qquad
L_{i,q}^{\mathrm{obs}}
=F_{i,q}^{\mathrm{obs}}-Q_{i,q}^{A,\mathrm{serv,act}}.
\end{gathered}
...
=\min\!\left\{|\mathcal R_{i,q}|,L_{i,q}^{\mathrm{obs}}\right\},
...
\end{align}
\end{subequations}
实际预约先按“已发布固定承诺、本轮新到预约”分级，再按 O-D 与用户编号排序；前 $Q_{i,q}^{A,\mathrm{serv,act}}$ 个预约使用编号最小的满电槽，余下预约显式记为违约。若存在预约违约，则 $L_{i,q}^{\mathrm{obs}}=0$，实际随机用户不会越级。首次实际违约后，该用户的后续预约事件被取消并进入补偿状态；未服务预约不产生换电收入，也不把退回 SOC 写入站内库存。$u_1,\ldots,u_{|\mathcal R_{i,q}|}$ 按实际到站时间排序，同一时刻按固定用户编号打破并列。为使真实逐槽转移唯一，定义确定性执行算子 $\operatorname{Exec}_q$：与 MPC 的规范化匹配一致，先处理实际预约服务前缀，再按上述顺序处理实际随机用户，并依次使用编号最小的可用槽位。槽位未换电时保留 $S_{i,b,q}^{\mathrm{pre,obs}}$，换电时改为相应事件携带的真实退回 SOC。用粗体记号汇集全部站点、槽位或用户分量，于是
\begin{equation}
\begin{aligned}
(\mathbf S_{q}^{\mathrm{obs}},\mathbf z_{R,q}^{\mathrm{act}},
\mathbf f_{A,q}^{\mathrm{act}})
...
(1-\rho_\varepsilon^{\mathrm{act}})s_\varepsilon^{A,\mathrm{act}},
...
Q_{i,\ell}^{A,\mathrm{fail,act}}.
\label{eq:realized_reservation_failure_cost}
\end{align}
因此，MPC 目标中的预测随机收益、未来预约违约风险和未来时段成本不会被当作真实 reward；$C_{\mathrm{adj},\ell}$ 只统计本轮正式发布且偏离日前基准路径的用户，$C_{\mathrm{fail},\ell}$ 也只统计首期真实发生的预约违约。未来用户在 MPC 中的调整成本和预测违约成本只是前瞻评价，不会在当期 reward 中重复实现。该 reward 使 actor 学习逐电池连续功率对当前成本、预约可靠性、路径稳定性和长期电池状态价值的共同影响。
