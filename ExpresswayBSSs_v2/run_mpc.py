# -*- coding: utf-8 -*-
r"""
当前公开入口使用 src.paper_mpc.solve_paper_mpc：它依据
paper/main.tex 以本机 Gurobi 联合求解六站路径流、路径调整、
预约结果与站级连续事件模式；RL 功率和终端参数仍来自 Mock。
旧离散模型和 EVENT_PATH_ENUM_REPLAY 说明仅作为历史迁移记录，
不再描述 run_rolling_mpc 的当前行为。
...
流程
----
当前公开入口 ``main`` / ``run_rolling_mpc`` 使用
``src.continuous_runner.run_continuous_rolling_mpc``：它加载或生成六站
schema-2 synthetic/mock 输入和连续日前计划，以固定 Mock 请求功率运行
``EVENT_PATH_ENUM_REPLAY``：从实时虚拟起点枚举候选剩余路径，以共享连续
事件内核回放评分并将赢家写入真实执行。它输出 schema-3 的
``continuous_event_mock_path_search`` 结果。所有外部字段和输出都来自带
seed 的模拟数据；此入口不训练或加载 RL 模型，也不声明联合事件位置 MILP
的全局最优性。

以下编号细节仅描述保留在本文件中的私有
``_run_legacy_rolling_mpc`` 迁移代码，不是当前公开执行语义。
...
当前公开结果输出 ``schema_version = 3``，并包含 ``run_mode =
"continuous_event_mock_path_search"``、半开连续时间服务策略、逐轮
``EVENT_PATH_ENUM_REPLAY`` 路径搜索摘要和真实事件账本。下列 schema-2
用户字段说明只适用于私有 legacy 分支：
...
def run_rolling_mpc(
    ...
    """Run the schema-3 continuous-event Mock execution path.

    ``_run_legacy_rolling_mpc`` remains private only to keep historical source
    context available during migration.  New data is schema 2/3 and must not
    flow through the old period-batched Gurobi branch.
    """
