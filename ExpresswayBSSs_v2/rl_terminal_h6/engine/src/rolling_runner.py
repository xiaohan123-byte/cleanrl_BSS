...
def run_rolling_mpc(params: Any, scenario: Any, network: dict | None = None,
    ...
    for ell in range(len(rounds), execution_period_limit(params)):
        ...
        try:
            ...
            if state_features is not None:
                record["state_input" if isinstance(state_features, dict) else "state_features"] = state_features
            if journal is not None:
    ...
    if feature_spec is not None and hasattr(feature_spec, "raw_state"):
        result["terminal_state_input"] = feature_spec.raw_state(state)
    if journal is not None:
