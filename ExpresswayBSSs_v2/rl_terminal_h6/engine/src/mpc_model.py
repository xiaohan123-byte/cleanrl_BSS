...
def solve_mpc(params: Any, window: MPCWindow, *, terminal_model=None, feature_spec=None,
    ...
    if terminal_model is not None and hasattr(terminal_model, "validate_params"):
        terminal_model.validate_params(params)
    ...
    try:
        ...
        if terminal_model is not None and terminal_model.kind == "learned_set_relu":
            terminal_expression, terminal_expressions, terminal_bounds, terminal_records = terminal_model.build_mip(
                model, cp, COPT, window, soc, y, total, arrival_cases, service, failures, deadline, feature_spec)
        elif terminal_model is not None and ell + horizon < params.num_periods:
        ...
        if terminal_model is not None:
            ...
            if numeric_features:
                if hasattr(terminal_model, "audit_mip"):
                    diagnostics.update(terminal_model.audit_mip(terminal_records, value, numeric_features))
