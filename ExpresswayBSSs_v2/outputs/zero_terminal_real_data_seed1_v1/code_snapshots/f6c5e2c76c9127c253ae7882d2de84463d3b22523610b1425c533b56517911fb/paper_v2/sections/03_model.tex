...
\subsection{候选路径网络}
\label{sec:network}
对每个 O--D 对 $p$，沿行驶方向排列入口、换电站和出口。路径
$o^{p,k}\to i_1\to\cdots\to i_q\to d^p$
表示用户依次在 $i_1,\ldots,i_q$ 换电，弧所跨越的其他站点不被访问。以 $D_{i,j}^p$ 和 $v_{i,j}^p$ 表示节点间距离和行驶 SOC 消耗，以 $\underline s_d^p$ 表示出口 SOC 下限。每次换电后车辆满电，因此站间弧满足 $v_{i,j}^p\le1$，站点至出口的弧满足 $v_{i,d^p}^p+\underline s_d^p\le1$；这些弧可预先生成并在各用户、各轮之间复用。

本轮网络按用户当前可用的信息构造。尚未进入高速的用户以入口为起点，使用已上报或预测的入口 SOC；正在行驶的用户以当前位置为虚拟起点，使用实时观测 SOC。记相应起点和 SOC 为 $o^{p,k}$ 和 $s_{\mathrm{dep}}^{p,k}$，仅当 $v_{o^{p,k},j}^{p,k}\le s_{\mathrm{dep}}^{p,k}$ 时连接起点与下游站点 $j$。当前在站等待的用户以该站为起点，后续弧按换电后的满电状态生成；当前等待请求单独保留，后续服务须先完成该次换电，具体见第~\ref{sec:model}~节。本轮网络由起点、其下游换电站及出口组成。
...
\end{equation}
其中，$v_{o^{p,k},d^p}^{p,k}$ 为从当前起点直接驶向出口的 SOC 消耗。两种情况均保证车辆到达出口时满足 SOC 下限。

在上述网络中枚举起点至出口的完整路径，再用最小换电间距 $\underline D$ 筛去过近的换电安排。对距离小于 $\underline D$ 的首段弧和站间弧，按距离从小到大考察，距离相同时按固定节点顺序处理；只有删除该弧及包含它的路径后仍有完整路径，才予以删除。首段换电间距从入口或最近一次实际换电位置计算，不以随车辆移动的虚拟起点计算；当前在站等待的用户，其后续首段从当前站计算。到出口的末段不参与间距剪枝。若已有保留计划（未进入高速）或已发布路径（在途）的剩余部分仍满足续航条件，剪枝时保留其全部弧，是否调整仍由路径更新约束决定。

合并最终保留路径中的弧，得到用户本轮的可用弧集 $\mathcal A^{p,k}$ 及节点集合 $\mathcal V^{p,k}$。只要原始网络存在完整可达路径，剪枝后就至少保留一条。这里的可达性仅指道路与车辆续航条件，能否按时获得满电电池由后续优化模型决定。

\subsection{日前换电路径计算}
\label{sec:dayahead}
根据预约信息中的 O--D 和入口 SOC，按第~\ref{sec:network}~节构造日前候选网络。设所得完整路径集合为 $\mathcal Y^{p,k}$，其元素由弧指示量 $y$ 表示，则日前路径取为
\begin{equation}
