...
def _bound_tolerance(params, limit):
    """Bound checks also allow floating-point residuals that are negligible physically.

    The solver's FeasTol is absolute; returned values can exceed it by small
    residuals (observed 1.8e-8 kW on a 60 kW bound). A relative epsilon of 1e-6
    of the bound (60 kW -> 0.06 W) stays orders of magnitude below any real
    modelling error, and accepted values are clamped to the bound afterwards.
    """
    return max(_solver_feasibility_tolerance(params), 1e-6 * float(limit))
...
def _validate_power(params, state, solution):
    ...
    for i in range(params.station.num_stations):
        ...
        for b in range(slots_at(params, i)):
            ...
            tolerance = _bound_tolerance(params, params.slot_power_limit(i, b))
            ...
            row.append(power)
        station_tolerance = _bound_tolerance(params, params.station_power_limit(i))
        if sum(row) > params.station_power_limit(i) + station_tolerance:
            raise ExecutionError(
                f"station power limit exceeded: station={i} total={sum(row)!r} "
                f"limit={params.station_power_limit(i)} tolerance={station_tolerance}")
        powers.append(row)
    return powers
